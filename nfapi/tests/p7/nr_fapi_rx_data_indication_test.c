 ...
 */
/*! \file nfapi/tests/p7/nr_fapi_rx_data_indication_test.c
 * \brief
...
static size_t get_rx_data_indication_size(nfapi_nr_rx_data_indication_t *msg)
{
  ...
  for (int pdu_idx = 0; pdu_idx < msg->number_of_pdus; ++pdu_idx) {
    nfapi_nr_rx_data_pdu_t *pdu = &msg->pdu_list[pdu_idx];
...
{
  size_t message_size = get_rx_data_indication_size(req);
