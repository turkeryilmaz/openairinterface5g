 ...
 *
 * -#    The VNF_P5_CLIENT is created. If it left to the VRAN partner to define how that is done. The client is responsiable to
 creation of the thread with the correct priorities within the wider system.
 * -#    The VNF_P5_CLIENT creates and initializes the VNF_P5_LIB (nfapi_vnf_config_create & nfapi_vnf_start) providing the address
 that it should listen on for incoming SCTP connections. It is expected that this would be provided by some VNF management system.
 The VNF_P5_CLIENT also provides information on the callbacks that the VNF_P5_LIB will use to inform the VNF_P5_CLIENT of the
 received messages
 * \code
 ...
 * -#    The PNF_P5_CLIENT is created and then creates the PNF_P5_LIB (nfapi_pnf_init & nfapi_pnf_start) passing the address of the
 VNF to connect to. The PNF_P5_CLIENT also provides information on the callbacks that the PNF_P5_LIB will use to inform the
 PNF_P5_CLIENT of the received messages
 * \code
 ...
 * -#    The VNF_P5_LIB receives the STCP connection request and indicates this to the VNF_P5_CLIENT (pnf_connection_indication).
 The VNF_P5_CLIENT can decide to accept or reject the connection.
  * \code
 ...
 * -#    The VNF and PNF then proceed to exchange the NFAPI PNF messages (PNF_PARAM, PNF_CONFIG and PNF_START). At each stage the
 LIB’s invoke callbacks on the CLIENT’s for them to handle message the return the appropriate response.
 * -#    Finally the VNF will send the PNF_START.req which will invoke the PNF_P5_CLIENT nfapi_pnf_start_request callback. This is
 the point at which it is expect that the PNF_CLIENT will create the FAPI module. The PNF_P5_CLIENT will need to perform the
 translation between NFAPI structures and vendor specific FAPI structures for messages sent between the PNF_P5_LIB and the FAPI
 interface.
 * -#    The PNF_START.response will sent back to the VNF and at this point the VNF_P5_CLIENT will need to decide which VNF_P7
 instance will handle the P7 connection. i.e. it needs to determine the IP address and port number of the P7 VNF instance.
 * -#    A new VNF_P7_CLIENT & VNF_P7_LIB should be configured and started if necessary.
 * -#    The VNF_P5 will then send the PARAM.request to the PNF_P5 which will forward it onto the FAPI interface. This PARAM.request
 includes the VNF_P7 address. The PNF_P7_CLIENT must decide on the PNF P7 address. This information is used to create and initialize
 the PNF_P7_LIB. The PNF_P7 address is then returned by the PNF_P5 to the VNF_P5 in the PARAM.response.
 * -#    The VNF_P5 will then exchange with the PNF_P5 the CONFIG.request/response which will be used to configure the PNF FAPI
 instance.
 * -#    The VNF_P5 will then decide to start the PNF_P7 by sending the START.request.
 * -#    The PNF_P5_CLIENT need to send the start request to the FAPI instance. The response which is the start of subframe
 indications from the FAPI. In receipt of the first subframe indication the PNF_P5_CLIENT will send the START.response to the VNF_P5
 via the PNF_P5_LIB
 * -#    The FAPI subframe indications should be forwarded to the PNF_P7_LIB by the PNF_P7_CLIENT. Until the VNF_P7 instance has
 sent the subframe configuration messages (dl_config, ul_config, etc) the PNF_P7_LIB will send ‘dummy’ subframe configuration
 messages. The contents of which are configurable by the PNF_P7_CLIENT.
 * -#    When the VNF_P5_CLIENT receives the START.response it will need to ‘communicate’ with the VNF_P7_CLIENT to inform it that
 the PNF_P7 instance has started. The VNF_P7_CLIENT will call the nfapi_vnf_P7_add_pnf function passing the address details of the
 PNF_P7 instance.
 * -#    The VNF_P7_LIB will then start the sync procedure to establish sub frame synchronization between the PNF_P7 and VNF_P7
 instances. This involves sending the DL_NODE_SYNC and UL_NODE_SYNC to determine network latency and PNF processing latency to be
 able to request MAC generate sub frames in advance on when they are required by the FAPI interface.
 * -#    When sync is achieved the VNF_P7_LIB will send the nfapi_sync_indication to the VNF_P7_CLIENT.
 * -#    The VNF_P7_LIB will then start issuing sub frame indications to the VNF_P7_CLIENT. The logical intent is that they
 indications are sent every millisecond. However due to the scheduling jitter that may be seen by the VNF these subframe indications
 may be less than or more than 1ms apart. How this is handled is one of the critical functions of the VNF_P7_LIB and may require
 specialization or requirements on the VNF environment i.e. CPU pinning.
 * -#    The VNF_P7_LIB will send the subframe_indication to the VNF_P7_CLIENT for ‘x’ subframe’s inadvance of the current TTI at
 the PNF. The delta ‘x’ will be determined by the sync procedure and how far in advance the FAPI needs to receive messages before RF
 transmission.
 * -#    The VNF_P7_CLIENT is then responsible for communicating to the MAC layer to prepare the dl_config_request,
 ul_config_request, hi_dci0_request, tx_req in a timely manner and sending them to the VNF_P7_LIB
 * -#    The VNF_P7_LIB will send them to the VNF_P7_LIB.
 * -#    The PNF_P7_LIB will store these messages in a subframe buffer or playout buffer in advance of when they are required by
 FAPI.
 *  -#    The PNF_P7_LIB will monitor to see if these messages arrive too late and if they do trigger a TIMING_INFO response to the
 VNF_P7_LIB to reassess if the sync is still valid.
 * -#    As some time in the future the FAPI will send a subframe indication for the TTI that the VNF_P7_LIB had previously
 requested.
 * -#    The PNF_P7_LIB will then use the messages in the subframe buffer and send them too the FAPI interface for transmission. The
 PNF_P7_CLIENT will need to perform the translation between NFAPI structure and vendor specific FAPI structures
