 ...
 public:
  phy_info()
      : first_subframe_ind(0),
        fapi(0),
        dl_ues_per_subframe(0),
        ul_ues_per_subframe(0),
        timing_window(0),
        timing_info_mode(0),
        timing_info_period(0)
  {
    index = 0;
    id = 0;
    udp = 0;
...
{
 public:
  pnf_info() : release(13), wireshark_test_mode(0), max_total_power(0), oui(0)
...
int fapi_param_response(fapi_t* fapi, fapi_param_resp_t* resp)
{
  ...
  if (1) {
    ...
    nfapi_resp.num_tlv++;
    nfapi_resp.nb_iot_capability.nb_iot_operating_mode_capability.tl.tag =
        NFAPI_LAA_CAPABILITY_NB_IOT_OPERATING_MODE_CAPABILITY_TAG;
    ...
    nfapi_resp.sch_config.primary_synchronization_signal_epre_eprers.tl.tag =
        NFAPI_SCH_CONFIG_PRIMARY_SYNCHRONIZATION_SIGNAL_EPRE_EPRERS_TAG;
    nfapi_resp.num_tlv++;
    nfapi_resp.sch_config.secondary_synchronization_signal_epre_eprers.tl.tag =
    ...
    nfapi_resp.num_tlv++;
    nfapi_resp.srs_config.srs_acknack_srs_simultaneous_transmission.tl.tag =
    ...
    nfapi_resp.num_tlv++;
    nfapi_resp.uplink_reference_signal_config.cyclic_shift_1_for_drms.tl.tag =
    ...
    nfapi_resp.num_tlv++;
    nfapi_resp.tdd_frame_structure_config.special_subframe_patterns.tl.tag =
...
int fapi_subframe_ind(fapi_t* fapi, fapi_subframe_ind_t* resp)
{
  ...
  if (data->phy->first_subframe_ind == 0) {
    ...
    if (data->phy->udp.enabled) {
      fapi_start_data(fapi, data->phy->udp.rx_port, data->phy->udp.tx_addr.c_str(), data->phy->udp.tx_port);
    }
  }
...
int fapi_crc_ind(fapi_t* fapi, fapi_crc_ind_t* ind)
{
  ...
  if (((pnf_info*)(data->config->user_data))->wireshark_test_mode) {
    ...
    crc_ind.crc_indication_body.crc_pdu_list =
        (nfapi_crc_indication_pdu_t*)malloc(sizeof(nfapi_crc_indication_pdu_t) * ind->body.number_of_crcs);
...
int fapi_rx_ulsch_ind(fapi_t* fapi, fapi_rx_ulsch_ind_t* ind)
{
  ...
  if (1) //((pnf_info*)(data->config->user_data))->wireshark_test_mode)
  {
    ...
    rx_ind.rx_indication_body.rx_pdu_list =
        (nfapi_rx_indication_pdu_t*)malloc(sizeof(nfapi_rx_indication_pdu_t) * ind->body.number_of_pdus);
...
int param_request(nfapi_pnf_config_t* config, nfapi_pnf_phy_config_t* phy, nfapi_param_request_t* req)
{
  ...
  auto found = std::find_if(pnf->phys.begin(), pnf->phys.end(), [&](phy_info& item) { return item.id == req->header.phy_id; });
