  ...
  while (iterator != 0) {
    if (rx_hr_time - iterator->rx_hr_time > delta) {
      ...
      NFAPI_TRACE(NFAPI_TRACE_WARN,
                  "Deleting stale reassembly message (packet rx_hr_time %u current rx_hr_time %u delta %d us)\n",
                  iterator->rx_hr_time,
                  rx_hr_time,
                  delta);
...
void vnf_handle_ul_node_sync(void *pRecvMsg, int recvMsgLen, vnf_p7_t* vnf_p7)
{
  ...
  if (phy->filtered_adjust && (phy->sf_offset_filtered > 1e6 || phy->sf_offset_filtered < -1e6)) {
    ...
    NFAPI_TRACE(NFAPI_TRACE_ERROR,
                "%s - ADJUST TOO BAD - go out of filtered phy->sf_offset_filtered:%d\n",
                __FUNCTION__,
                phy->sf_offset_filtered);
  }
        ...
        if (phy->adjustment == 0) {
          ...
          if (phy->in_sync) {
            ...
            if (phy->insync_minor_adjustment != 0) {
              NFAPI_TRACE(NFAPI_TRACE_NOTE,
                          "(%4d/%d) VNF phy_id:%d Apply minor insync adjustment %dus for %d subframes (sf_offset_filtered:%d) %d "
                          "%d %d NEW:%d CURR:%d adjustment:%d\n",
                          NFAPI_SFNSF2SFN(phy->sfn_sf),
                          NFAPI_SFNSF2SF(phy->sfn_sf),
                          ind.header.phy_id,
...
void vnf_nr_handle_ul_node_sync(void *pRecvMsg, int recvMsgLen, vnf_p7_t* vnf_p7)
{
  ...
  uint32_t tx_2_rx = t4 > ind.t1 ? t4 - ind.t1
                                 : t4 + NFAPI_MAX_SFNSLOTDEC(phy->mu)
        ...
        if (phy->min_sync_cycle_count == 0) {
          ...
          if (!phy->filtered_adjust) {
            ...
            NFAPI_TRACE(NFAPI_TRACE_NOTE,
                        ...
                        phy->slot_offset,
                        NFAPI_SFNSLOTDEC2SFN(phy->mu, sfn_slot_dec),
                        NFAPI_SFNSLOTDEC2SLOT(phy->mu, sfn_slot_dec));
    ...
    return;
        ...
        if (vnf_p7 && vnf_p7->p7_connections)
        {
          ...
          //NFAPI_TRACE(NFAPI_TRACE_INFO, "%s() PNF:SFN/SF:%d VNF:SFN/SF:%d deltaSFNSF:%d\n", __FUNCTION__, NFAPI_SFNSF2DEC(ind.last_sfn_sf), NFAPI_SFNSF2DEC(vnf_p7->p7_connections[0].sfn_sf), vnf_pnf_sfnsf_delta);
            ...
            {
              NFAPI_TRACE(NFAPI_TRACE_WARN,
                          "%s() LARGE SFN/SLOT DELTA between PNF and VNF. Delta %d slots. PNF:%d.%d VNF:%d.%d\n",
                          __FUNCTION__,
