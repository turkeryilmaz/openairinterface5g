...
class Log_Mgt:
	...
	def __init__(self, IPAddress, Path):
...
#-----------------$
	...
	def __CheckUsedSpace(self):
		HOST=f'{self.IPAddress}'
		COMMAND=f"df {self.path}"
	...
	def __RemoveOldest(self, days):
		...
		COMMAND=f'sudo find {self.path} -type f -mtime {days} -delete'
		cmd.run(COMMAND)
		cmd.close()
...
#-----------------$
	...
	def LogRotation(self):
		...
		while doLoop and nbDays > 1:
			...
			if used_space > 80 :
				logging.debug(f'\u001B[1;37;41m  Used Disk ({used_space}%) > 80%, on {self.IPAddress}\u001B[0m')
				logging.debug(f'\u001B[1;37;41m  Removing Artifacts older than {nbDays} days\u001B[0m')
				self.__RemoveOldest(nbDays)
				nbDays -= 1
			else:
				logging.debug(f'Used Disk ({used_space}%) < 80%, on {self.IPAddress}, no cleaning required')
				doLoop = False
