...
class LocalCmd(Cmd):
	...
	def command(self, commandline, expectedline, timeout, silent=False, resync=False):
		logging.warning("LocalCmd emulating sshconnection.command() function")
...
class RemoteCmd(Cmd):
	...
	def cd(self, d, silent=False):
		if d == None or d == '' or d == []:
			return
		cmd = f'cd {d}'
		stdin, stdout, stderr = self.client.exec_command(msg)
		if not silent:
			logging.debug(msg)
	...
	def run(self, line, timeout=300, silent=False, reportNonZero=True):
		...
		ret.stdout = ret.stdout.decode('utf-8')
		if reportNonZero and ret.returncode != 0:
			logging.warning(f'command "{ret.args}" return non-zero returncode {ret.returncode}: output:\n{ret.stdout}')
		self.cp = ret
		return ret

	def command(self, commandline, expectedline, timeout, silent=False, resync=False):
		logging.warning("RemoteCmd emulating sshconnection.command() function")
	...
	def copyout(self, scpIp, scpUser, scpPw, src, tgt):
		logging.warning("RemoteCmd emulation sshconnection.copyout() function, ignoring scpIp")
	...
	def copyin(self, scpIp, scpUser, scpPw, src, tgt):
		logging.warning("RemoteCmd emulation sshconnection.copyout() function")
