  ...
  if (sctp_shutdown) {
    ...
    if (mme_desc_p->state == S1AP_ENB_STATE_CONNECTED) {
      ...
      if (mme_desc_p->s1ap_eNB_instance->s1ap_mme_associated_nb == 0) {
        ...
        message_p = S1AP_DEREGISTERED_ENB_IND_alloc(TASK_S1AP, 0);
        S1AP_DEREGISTERED_ENB_IND_data(message_p)->nb_mme = 0;
    ...
    if (mme_desc_p->s1ap_eNB_instance->s1ap_mme_pending_nb == 0) {
      MessageDef                 *message_p;
      message_p = S1AP_REGISTER_ENB_CNF_alloc(TASK_S1AP, 0);
      S1AP_REGISTER_ENB_CNF_data(message_p)->nb_mme = mme_desc_p->s1ap_eNB_instance->s1ap_mme_associated_nb;
      itti_send_msg_to_task(TASK_ENB_APP, mme_desc_p->s1ap_eNB_instance->instance, message_p);
    }
  }
}
...
int s1ap_eNB_handle_initial_context_request(uint32_t   assoc_id,
  ...
  message_p = S1AP_INITIAL_CONTEXT_SETUP_REQ_alloc(TASK_S1AP, 0);
  s1ap_initial_context_setup_req_t * msg=S1AP_INITIAL_CONTEXT_SETUP_REQ_data(message_p);
  msg->ue_initial_id  = ue_desc_p->ue_initial_id;
  ue_desc_p->ue_initial_id = 0;
  msg->eNB_ue_s1ap_id = ue_desc_p->eNB_ue_s1ap_id;
  msg->mme_ue_s1ap_id = ue_desc_p->mme_ue_s1ap_id;
  ...
  if (ie != NULL) { /* checked by macro but cppcheck doesn't see it */
    ...
    for (i = 0; i < ie->value.choice.E_RABToBeSetupListCtxtSUReq.list.count; i++) {
      ...
      msg->e_rab_param[i].e_rab_id = item_p->e_RAB_ID;

      if (item_p->nAS_PDU != NULL) {
        /* Only copy NAS pdu if present */
        msg->e_rab_param[i].nas_pdu.length = item_p->nAS_PDU->size;
        msg->e_rab_param[i].nas_pdu.buffer =
          malloc(sizeof(uint8_t) * item_p->nAS_PDU->size);
        memcpy(msg->e_rab_param[i].nas_pdu.buffer,
               item_p->nAS_PDU->buf, item_p->nAS_PDU->size);
        S1AP_DEBUG("Received NAS message with the E_RAB setup procedure\n");
      } else {
        msg->e_rab_param[i].nas_pdu.length = 0;
        msg->e_rab_param[i].nas_pdu.buffer = NULL;
      }

      /* Set the transport layer address */
      memcpy(msg->e_rab_param[i].sgw_addr.buffer,
             item_p->transportLayerAddress.buf, item_p->transportLayerAddress.size);
      msg->e_rab_param[i].sgw_addr.length =
        item_p->transportLayerAddress.size * 8 - item_p->transportLayerAddress.bits_unused;
      /* GTP tunnel endpoint ID */
      OCTET_STRING_TO_INT32(&item_p->gTP_TEID, msg->e_rab_param[i].gtp_teid);
      /* Set the QOS informations */
      msg->e_rab_param[i].qos.qci = item_p->e_RABlevelQoSParameters.qCI;
      msg->e_rab_param[i].qos.allocation_retention_priority.priority_level =
      ...
      msg->e_rab_param[i].qos.allocation_retention_priority.pre_emp_vulnerability =
  ...
  if (ie != NULL) { /* checked by macro but cppcheck doesn't see it */
    msg->security_capabilities.encryption_algorithms =
  ...
  if (ie != NULL) {
    msg->nr_security_capabilities.encryption_algorithms =
      BIT_STRING_to_uint16(&ie->value.choice.NRUESecurityCapabilities.nRencryptionAlgorithms);
    msg->nr_security_capabilities.integrity_algorithms =
      BIT_STRING_to_uint16(&ie->value.choice.NRUESecurityCapabilities.nRintegrityProtectionAlgorithms);
  } else {
    msg->nr_security_capabilities.encryption_algorithms = 0;
    msg->nr_security_capabilities.integrity_algorithms = 0;
  }
    ...
    switch (ie->value.choice.UE_S1AP_IDs.present) {
      case S1AP_UE_S1AP_IDs_PR_uE_S1AP_ID_pair:
        ...
        if ((ue_desc_p = s1ap_eNB_get_ue_context(mme_desc_p->s1ap_eNB_instance,
          ...
          S1AP_UE_CONTEXT_RELEASE_COMMAND_data(message_p)->eNB_ue_s1ap_id = enb_ue_s1ap_id;
          itti_send_msg_to_task(TASK_RRC_ENB, ue_desc_p->eNB_instance->instance, message_p);
          return 0;
        }
...
int s1ap_eNB_handle_e_rab_setup_request(uint32_t         assoc_id,
  ...
  message_p = S1AP_E_RAB_SETUP_REQ_alloc(TASK_S1AP, 0);
  s1ap_e_rab_setup_req_t *msg=S1AP_E_RAB_SETUP_REQ_data(message_p);
  msg->ue_initial_id  = ue_desc_p->ue_initial_id;
  ...
  if (ie != NULL) { /* checked by macro but cppcheck doesn't see it */
    ...
    for (i = 0; i < ie->value.choice.E_RABToBeSetupListBearerSUReq.list.count; i++) {
      ...
      msg->e_rab_setup_params[i].e_rab_id = item_p->e_RAB_ID;

      // check for the NAS PDU
      if (item_p->nAS_PDU.size > 0 ) {
        msg->e_rab_setup_params[i].nas_pdu.length = item_p->nAS_PDU.size;
        msg->e_rab_setup_params[i].nas_pdu.buffer = malloc(sizeof(uint8_t) * item_p->nAS_PDU.size);
        memcpy(msg->e_rab_setup_params[i].nas_pdu.buffer,
               item_p->nAS_PDU.buf, item_p->nAS_PDU.size);
        // S1AP_INFO("received a NAS PDU with size %d (%02x.%02x)\n",msg->e_rab_setup_params[i].nas_pdu.length, item_p->nAS_PDU.buf[0], item_p->nAS_PDU.buf[1]);
      } else {
        msg->e_rab_setup_params[i].nas_pdu.length = 0;
        msg->e_rab_setup_params[i].nas_pdu.buffer = NULL;
        S1AP_WARN("NAS PDU is not provided, generate a E_RAB_SETUP Failure (TBD) back to MME \n");
      }

      /* Set the transport layer address */
      memcpy(msg->e_rab_setup_params[i].sgw_addr.buffer,
             item_p->transportLayerAddress.buf, item_p->transportLayerAddress.size);
      msg->e_rab_setup_params[i].sgw_addr.length =
        item_p->transportLayerAddress.size * 8 - item_p->transportLayerAddress.bits_unused;
      /* S1AP_INFO("sgw addr %s  len: %d (size %d, index %d)\n",
                   msg->e_rab_setup_params[i].sgw_addr.buffer,
                   msg->e_rab_setup_params[i].sgw_addr.length,
                   item_p->transportLayerAddress.size, i);
      */
      /* GTP tunnel endpoint ID */
      OCTET_STRING_TO_INT32(&item_p->gTP_TEID, msg->e_rab_setup_params[i].gtp_teid);
      /* Set the QOS informations */
      msg->e_rab_setup_params[i].qos.qci = item_p->e_RABlevelQoSParameters.qCI;
      msg->e_rab_setup_params[i].qos.allocation_retention_priority.priority_level =
        item_p->e_RABlevelQoSParameters.allocationRetentionPriority.priorityLevel;
      msg->e_rab_setup_params[i].qos.allocation_retention_priority.pre_emp_capability =
        item_p->e_RABlevelQoSParameters.allocationRetentionPriority.pre_emptionCapability;
      msg->e_rab_setup_params[i].qos.allocation_retention_priority.pre_emp_vulnerability =
        item_p->e_RABlevelQoSParameters.allocationRetentionPriority.pre_emptionVulnerability;
    } /* for i... */
...
int s1ap_eNB_handle_paging(uint32_t               assoc_id,
  ...
  message_p = S1AP_PAGING_IND_alloc(TASK_S1AP, 0);
  s1ap_paging_ind_t *msg=S1AP_PAGING_IND_data(message_p);
  ...
  if (ie != NULL) { /* checked by macro but cppcheck doesn't see it */
    msg->ue_index_value  = BIT_STRING_to_uint32(&ie->value.choice.UEIdentityIndexValue);
    S1AP_DEBUG("[SCTP %u] Received Paging ue_index_value (%u)\n",
               assoc_id,(uint32_t)msg->ue_index_value);
    msg->ue_paging_identity.choice.s_tmsi.mme_code = 0;
    msg->ue_paging_identity.choice.s_tmsi.m_tmsi = 0;
  } else {
    itti_free(ITTI_MSG_ORIGIN_ID(message_p), message_p);
    return -1;
  }
  ...
  if (ie != NULL) { /* checked by macro but cppcheck doesn't see it */
    ...
    if (ie->value.choice.UEPagingID.present == S1AP_UEPagingID_PR_s_TMSI) {
      msg->ue_paging_identity.presenceMask = UE_PAGING_IDENTITY_s_tmsi;
      OCTET_STRING_TO_INT8(&ie->value.choice.UEPagingID.choice.s_TMSI.mMEC, msg->ue_paging_identity.choice.s_tmsi.mme_code);
      OCTET_STRING_TO_INT32(&ie->value.choice.UEPagingID.choice.s_TMSI.m_TMSI, msg->ue_paging_identity.choice.s_tmsi.m_tmsi);
    } else if (ie->value.choice.UEPagingID.present == S1AP_UEPagingID_PR_iMSI) {
      msg->ue_paging_identity.presenceMask = UE_PAGING_IDENTITY_imsi;
      msg->ue_paging_identity.choice.imsi.length = 0;

      for (int i = 0; i < ie->value.choice.UEPagingID.choice.iMSI.size; i++) {
        msg->ue_paging_identity.choice.imsi.buffer[2*i] = (uint8_t)(ie->value.choice.UEPagingID.choice.iMSI.buf[i] & 0x0F );
        msg->ue_paging_identity.choice.imsi.length++;
        msg->ue_paging_identity.choice.imsi.buffer[2*i+1] = (uint8_t)((ie->value.choice.UEPagingID.choice.iMSI.buf[i]>>4) & 0x0F);
        LOG_D(S1AP,"paging : i %d %d imsi %d %d \n",2*i,2*i+1,msg->ue_paging_identity.choice.imsi.buffer[2*i], msg->ue_paging_identity.choice.imsi.buffer[2*i+1]);

        if (msg->ue_paging_identity.choice.imsi.buffer[2*i+1] == 0x0F) {
  ...
  if (ie != NULL) { /* checked by macro but cppcheck doesn't see it */
    ...
    if (ie->value.choice.CNDomain == S1AP_CNDomain_ps) {
      msg->cn_domain = CN_DOMAIN_PS;
    } else if (ie->value.choice.CNDomain == S1AP_CNDomain_cs) {
      msg->cn_domain = CN_DOMAIN_CS;
    } else {
  ...
  S1AP_DEBUG("[SCTP %u] Received Paging parameters: ue_index_value %d  cn_domain %d paging_drx %d paging_priority %d\n",assoc_id,
             msg->ue_index_value, msg->cn_domain,
  ...
  S1AP_DEBUG("[SCTP %u] Received Paging parameters(ue): presenceMask %d  s_tmsi.m_tmsi %d s_tmsi.mme_code %d IMSI length %d (0-5) %d%d%d%d%d%d\n",assoc_id,
             msg->ue_paging_identity.presenceMask, msg->ue_paging_identity.choice.s_tmsi.m_tmsi,
             msg->ue_paging_identity.choice.s_tmsi.mme_code, msg->ue_paging_identity.choice.imsi.length,
             msg->ue_paging_identity.choice.imsi.buffer[0], msg->ue_paging_identity.choice.imsi.buffer[1],
             msg->ue_paging_identity.choice.imsi.buffer[2], msg->ue_paging_identity.choice.imsi.buffer[3],
             msg->ue_paging_identity.choice.imsi.buffer[4], msg->ue_paging_identity.choice.imsi.buffer[5]);
  /* send message to RRC */
...
int s1ap_eNB_handle_e_rab_modify_request(uint32_t               assoc_id,
  ...
  if (ue_desc_p->mme_ue_s1ap_id != mme_ue_s1ap_id) {
    S1AP_WARN("UE context mme_ue_s1ap_id is different form that of the message (%u != %ld)",
              ue_desc_p->mme_ue_s1ap_id, mme_ue_s1ap_id);
    message_p = S1AP_E_RAB_MODIFY_RESP_alloc(TASK_RRC_ENB, 0);
    s1ap_e_rab_modify_resp_t * msg=S1AP_E_RAB_MODIFY_RESP_data(message_p);
    msg->eNB_ue_s1ap_id = enb_ue_s1ap_id;
    S1AP_FIND_PROTOCOLIE_BY_ID(S1AP_E_RABModifyRequestIEs_t, ie, container,
    ...
    if (ie != NULL) { /* checked by macro but cppcheck doesn't see it */
      for(nb_of_e_rabs_failed = 0; nb_of_e_rabs_failed < ie->value.choice.E_RABToBeModifiedListBearerModReq.list.count; nb_of_e_rabs_failed++) {
        ...
        msg->e_rabs_failed[nb_of_e_rabs_failed].e_rab_id = item_p->e_RAB_ID;
        msg->e_rabs_failed[nb_of_e_rabs_failed].cause = S1AP_CAUSE_RADIO_NETWORK;
        msg->e_rabs_failed[nb_of_e_rabs_failed].cause_value = S1AP_CauseRadioNetwork_unknown_mme_ue_s1ap_id;
      }
    } else {
      itti_free(ITTI_MSG_ORIGIN_ID(message_p), message_p);
      return -1;
    ...
    msg->nb_of_e_rabs_failed = nb_of_e_rabs_failed;
    s1ap_eNB_e_rab_modify_resp(mme_desc_p->s1ap_eNB_instance->instance,
                               S1AP_E_RAB_MODIFY_RESP_data(message_p));
    itti_free(TASK_RRC_ENB,message_p);
    message_p = NULL;
    return -1;
  }

  message_p = S1AP_E_RAB_MODIFY_REQ_alloc(TASK_S1AP, 0);
  s1ap_e_rab_modify_req_t * msg=S1AP_E_RAB_MODIFY_REQ_data(message_p);
  msg->ue_initial_id  = ue_desc_p->ue_initial_id;
  msg->mme_ue_s1ap_id  = mme_ue_s1ap_id;
  msg->eNB_ue_s1ap_id  = enb_ue_s1ap_id;
  ...
  if (ie != NULL) { /* checked by macro but cppcheck doesn't see it */
    ...
    for (i = 0; i < ie->value.choice.E_RABToBeModifiedListBearerModReq.list.count; i++) {
      ...
      msg->e_rab_modify_params[i].e_rab_id = item_p->e_RAB_ID;

      // check for the NAS PDU
      if (item_p->nAS_PDU.size > 0 ) {
        msg->e_rab_modify_params[i].nas_pdu.length = item_p->nAS_PDU.size;
        msg->e_rab_modify_params[i].nas_pdu.buffer = malloc(sizeof(uint8_t) * item_p->nAS_PDU.size);
        memcpy(msg->e_rab_modify_params[i].nas_pdu.buffer,
               item_p->nAS_PDU.buf, item_p->nAS_PDU.size);
      } else {
        msg->e_rab_modify_params[i].nas_pdu.length = 0;
        msg->e_rab_modify_params[i].nas_pdu.buffer = NULL;
        continue;
      }

      /* Set the QOS informations */
      msg->e_rab_modify_params[i].qos.qci = item_p->e_RABLevelQoSParameters.qCI;
      msg->e_rab_modify_params[i].qos.allocation_retention_priority.priority_level =
        item_p->e_RABLevelQoSParameters.allocationRetentionPriority.priorityLevel;
      msg->e_rab_modify_params[i].qos.allocation_retention_priority.pre_emp_capability =
        item_p->e_RABLevelQoSParameters.allocationRetentionPriority.pre_emptionCapability;
      msg->e_rab_modify_params[i].qos.allocation_retention_priority.pre_emp_vulnerability =
        item_p->e_RABLevelQoSParameters.allocationRetentionPriority.pre_emptionVulnerability;
    }
...
int s1ap_eNB_handle_e_rab_release_command(uint32_t               assoc_id,
  ...
  message_p = S1AP_E_RAB_RELEASE_COMMAND_alloc(TASK_S1AP, 0);
  s1ap_e_rab_release_command_t * msg=S1AP_E_RAB_RELEASE_COMMAND_data(message_p);
  msg->eNB_ue_s1ap_id = enb_ue_s1ap_id;
  msg->mme_ue_s1ap_id = mme_ue_s1ap_id;
  ...
  if (ie != NULL) { /* checked by macro but cppcheck doesn't see it */
    msg->nb_e_rabs_torelease = ie->value.choice.E_RABList.list.count;
    ...
    for (i = 0; i < ie->value.choice.E_RABList.list.count; i++) {
      ...
      msg->e_rab_release_params[i].e_rab_id = item_p->e_RAB_ID;
...
int s1ap_eNB_handle_s1_path_switch_request_ack(uint32_t               assoc_id,
  ...
  message_p = S1AP_PATH_SWITCH_REQ_ACK_alloc(TASK_S1AP, 0);
  s1ap_path_switch_req_ack_t * msg=S1AP_PATH_SWITCH_REQ_ACK_data(message_p);
  /* mandatory */
  ...
  msg->eNB_ue_s1ap_id = ie->value.choice.ENB_UE_S1AP_ID;
  ...
  msg->mme_ue_s1ap_id = ie->value.choice.MME_UE_S1AP_ID;
  ...
  msg->next_hop_chain_count =
    ie->value.choice.SecurityContext.nextHopChainingCount;
  memcpy(&msg->next_security_key,
  ...
  if (ie) {
    msg->nb_e_rabs_tobeswitched = ie->value.choice.E_RABToBeSwitchedULList.list.count;
    ...
    for (int i = 0; i < ie->value.choice.E_RABToBeSwitchedULList.list.count; i++) {
      ...
      msg->e_rabs_tobeswitched[i].e_rab_id = s1ap_E_RABToBeSwitchedULItem->e_RAB_ID;
      memcpy(msg->e_rabs_tobeswitched[i].sgw_addr.buffer,
             s1ap_E_RABToBeSwitchedULItem->transportLayerAddress.buf, s1ap_E_RABToBeSwitchedULItem->transportLayerAddress.size);
      msg->e_rabs_tobeswitched[i].sgw_addr.length =
      ...
      OCTET_STRING_TO_INT32(&s1ap_E_RABToBeSwitchedULItem->gTP_TEID,
                            msg->e_rabs_tobeswitched[i].gtp_teid);
    }
  } else {
    S1AP_WARN("E_RABToBeSwitchedULList not supported\n");
    msg->nb_e_rabs_tobeswitched = 0;
  }
  ...
  if (ie) {
    msg->nb_e_rabs_tobereleased = ie->value.choice.E_RABList.list.count;
    ...
    for (int i = 0; i < ie->value.choice.E_RABList.list.count; i++) {
      ...
      msg->e_rabs_tobereleased[i].e_rab_id = e_RABItem->e_RAB_ID;
    }
  } else {
    S1AP_WARN("E_RABToBeReleasedList not supported\n");
    msg->nb_e_rabs_tobereleased = 0;
  }
