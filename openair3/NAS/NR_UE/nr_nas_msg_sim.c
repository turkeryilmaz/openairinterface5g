...
static char *get_sdap_pdu_if_name(const unsigned int ue_id, const unsigned int pdusession_id, char buf[IF_NAMESIZE])
{
  ...
  const unsigned int default_pdu = softmodem_nas_info.default_pdu_session_id;
  ...
  if (msg_p != NULL) {
    ...
    switch (ITTI_MSG_ID(msg_p)) {
      ...
      case NAS_DOWNLINK_DATA_IND: {
        ...
        switch (msg_type) {
          ...
          case FGS_DOWNLINK_NAS_TRANSPORT:
            decodeDownlinkNASTransport(&initialNasMsg, pdu_buffer, nas->UE_id);
            break;
      ...
      case NAS_INIT_NOS1_IF: {
        ...
        const char *ip = !softmodem_nas_info.nsa ? "10.0.1.2" : "10.0.1.3";
        const int qfi = 7;
        nr_ue_create_ip_if(ip, NULL, nas->UE_id + 1, pdu_session_id);
