...
static void ss_task_handle_rrc_pdu_req(struct NR_RRC_PDU_REQ *req)
{
        ...
        MessageDef *message_p = itti_alloc_new_message(TASK_RRC_GNB, instance_g, SS_RRC_PDU_REQ);
        assert(message_p);
        if (message_p)
        {
                ...
                LOG_A(GNB_APP, "[SS_SRB][NR_RRC_PDU_REQ] sending to TASK_RRC_GNB: {srb: %d, ch: %s, qty: %d }",
                          ...
                          req->RrcPdu.d == NR_RRC_MSG_Request_Type_Ccch ? "CCCH" : "DCCH", SS_RRC_PDU_REQ(message_p).sdu_size);
...
#endif
                        {
                                LOG_A(GNB_APP, "[SS_SRB] Received SS_RRC_PDU_IND from RRC\n");
                                if (RC.ss.State >= SS_STATE_CELL_ACTIVE)
                                {
