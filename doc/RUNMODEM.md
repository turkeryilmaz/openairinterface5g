...
| rfsim_serveraddr | string         |               | RF-Simulator Server Address          |
| rfsim_serverport | integer        | 0             | RF-Simulator Server Port             |
| rfsim_prop_delay | floating point | 0.0           | RF-Simulator Propagation Delay in ms |

The `cells` section in the NR UE configuration file contains an array of elements, where each element has these properies:

| Property name    | Type    | Default value | Description                              |
| ---------------- | ------- | ------------- | ---------------------------------------- |
| rf_port          | integer | 0             | ID of the associated RF-Port from the `RUs` section |
...
RUs = (
    {
        rfsim_serveraddr = "127.0.0.2";
        rfsim_serverport = 4043;
    }, {
        rfsim_serveraddr = "127.0.0.3";
        rfsim_serverport = 4044;
    }
);
