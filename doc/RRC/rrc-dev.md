...
sequenceDiagram
  ...
  alt HO Context not present
    Note over tcucp: rrc_gNB_generate_dedicatedRRCReconfiguration
  else
  ...
  Note over sdu: Stop scheduling UE
  Note over scucp: rrc_CU_process_ue_context_setup_response() ("on source CU")
...
for handover request acknowledge, `ho_success` for handover success, `ho_failure` for handover failure.
