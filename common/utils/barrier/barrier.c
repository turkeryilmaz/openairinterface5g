...
{
  AssertFatal(pthread_mutex_init(&barrier->barrier_lock, NULL) == 0, "");
...
{
  AssertFatal(pthread_mutex_lock(&barrier->barrier_lock) == 0, "");
  ...
  AssertFatal(pthread_mutex_unlock(&barrier->barrier_lock) == 0, "");
}
