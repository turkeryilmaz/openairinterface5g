...
_Thread_local int tpool_thread_index = -1;
...
void async_ws_task_manager(ws_task_manager_t* man, task_t t)
{
  ...
  for (size_t i = 0; i < len_thr; ++i) {
    int queue_index = (i + index) % len_thr;
    if (queue_index == tpool_thread_index) {
      continue;
    }
    if (try_push_not_q(&q_arr[queue_index], t)) {
      man->num_task += 1;
  ...
  int queue_index = index % len_thr;
  while (queue_index == tpool_thread_index) {
    queue_index = (queue_index + 1) % len_thr;
  }

  push_not_q(&q_arr[queue_index], t);
