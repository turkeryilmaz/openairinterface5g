...
static int environment_case(bool role_mismatch, bool sampled)
{
  ...
  char streams[sizeof(root) + sizeof("/streams")];
  char configuration_path[sizeof(root) + sizeof("/effective-config.json")];
  char opening_path[sizeof(root) + sizeof("/opening.bin")];
  CHECK(snprintf(streams, sizeof(streams), "%s/streams", root) > 0);
  CHECK(snprintf(configuration_path, sizeof(configuration_path), "%s/effective-config.json", root) > 0);
  CHECK(snprintf(opening_path, sizeof(opening_path), "%s/opening.bin", root) > 0);
  ...
  CHECK(setenv(environment_names[1], root, 1) == 0);
  CHECK(setenv(environment_names[2], configuration_path, 1) == 0);
  CHECK(setenv(environment_names[3], opening_path, 1) == 0);
  CHECK(setenv(environment_names[4], "7", 1) == 0);
  CHECK(setenv(environment_names[5], "1", 1) == 0);
  CHECK(setenv(environment_names[6], "8", 1) == 0);
  CHECK(setenv(environment_names[7], sampled ? "3" : "4", 1) == 0);
  ...
  CHECK(setenv(environment_names[8], sampled ? "64" : "0", 1) == 0);
  CHECK(setenv(environment_names[9], sample_seed_text, 1) == 0);
  CHECK(setenv(environment_names[10], sampled ? "1" : "0", 1) == 0);
  CHECK(setenv(environment_names[11], sampled ? "8" : "0", 1) == 0);
  CHECK(setenv(environment_names[12], "1", 1) == 0);
  CHECK(setenv(environment_names[13], "1", 1) == 0);
  CHECK(setenv(environment_names[14], "1000000", 1) == 0);
  CHECK(setenv(environment_names[15], "1000000000", 1) == 0);
  ...
  if (role_mismatch) {
    CHECK(start == OAI_MEMPROF_SOFTMODEM_SESSION_INVALID_CONFIGURATION);
  } else {
    ...
    char stream_path[sizeof(root) + sizeof("/streams/memory-lifetime.bin")];
    char handoff_path[sizeof(root) + sizeof("/streams/process-handoff.bin")];
    CHECK(snprintf(stream_path, sizeof(stream_path), "%s/streams/memory-lifetime.bin", root) > 0);
    CHECK(snprintf(handoff_path, sizeof(handoff_path), "%s/streams/process-handoff.bin", root) > 0);
    CHECK(unlink(stream_path) == 0 && unlink(handoff_path) == 0);
  }
  CHECK(unlink(configuration_path) == 0 && unlink(opening_path) == 0);
  CHECK(rmdir(streams) == 0 && rmdir(root) == 0);
  return EXIT_SUCCESS;
}
...
int main(int argc, char **argv)
{
  ...
  if (strcmp(argv[1], "disabled") == 0) {
    ...
    return environment_case(true, false);
  } else if (strcmp(argv[1], "positive") == 0) {
    return environment_case(false, false);
  } else if (strcmp(argv[1], "sampled") == 0) {
    return environment_case(false, true);
  } else {
    return 64;
  }
  return EXIT_SUCCESS;
