...
OAI_MEMPROF_WRAPPER_ATTRIBUTES char *__wrap_strdup(const char *source)
{
  ...
  const bool admitted = oai_memprof_active_runtime_begin_v1(11, 0, false, &ticket);
  char *result = __real_strdup(source);
  if (!admitted)
    return result;
