...
OAI_MEMPROF_WRAPPER_ATTRIBUTES char *__wrap_strndup(const char *source, size_t size)
{
  ...
  if (oai_memprof_active_control_state_v1(control) != OAI_MEMPROF_CORE_ACTIVE)
    return __real_strndup(source, size);

  oai_memprof_core_ticket_t ticket = {0};
  const bool admitted = oai_memprof_active_runtime_begin_v1(12, 0, false, &ticket);
  char *result = __real_strndup(source, size);
  if (!admitted)
    return result;
  ...
  const oai_memprof_core_payload_t payload = {
      ...
      .arg0 = (uint64_t)(uintptr_t)source,
      .arg1 = (uint64_t)size,
      .flags = flags,
