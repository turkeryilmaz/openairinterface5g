...
OAI_MEMPROF_WRAPPER_ATTRIBUTES void *__wrap_valloc(size_t size)
{
  ...
  const bool admitted = oai_memprof_active_runtime_begin_v1(9, (uint64_t)size, true, &ticket);
  void *result = __real_valloc(size);
  if (!admitted)
    return result;
