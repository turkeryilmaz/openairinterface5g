 ...
 * \brief T-Tracer UE service called by data recording application
 * \author Abdo Gaber
 * \date 2024
 * \version 1.0
...
typedef struct {
  ...
  int dmrs_nscid, nb_antennas_tx, number_of_bits;
  int data_size, data;
...
#define G(var_name, var_type, var) \
  ...
  * the value is an index in the event, see below */
  ...
  for (i = 0; i < f.count; i++) {
    ...
    G("nb_antennas_rx", "int", d->nb_antennas_tx) // bug need to be checked that why we are getting the right tx antennas only if set to rx "nb_antennas_rx"
...
restart:
  ...
  while (1) {
    ...
    if (isValueInArray(e.type, req_tracer_msgs_indices, num_req_tracer_msgs)) { 
      ...
      // For BITS messages: data in bytes
