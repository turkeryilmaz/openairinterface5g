...
static void log_output_memory(log_component_t *c,
                              const char *file,
                              const char *func,
                              int line,
                              int comp,
...
int logInit(void)
{
  ...
  register_log_component("NRPPA", "", NRPPA);
...
log_header(log_component_t *c, char *log_buffer, int buffsize, const char *file, const char *func, int line, int level)
{
  ...
  if ((flag & FLAG_TIME) || (flag & FLAG_REAL_TIME)) {
    ...
    if (clock_gettime(clock, &t) == -1)
      abort();
    snprintf(timeString, sizeof(timeString), "%lu.%06lu ", t.tv_sec, t.tv_nsec / 1000);
  } else {
    timeString[0] = 0;
  }
...
int set_log(int component, int level)
{
  ...
  DevCheck((component >= MIN_LOG_COMPONENTS) && (component < MAX_LOG_COMPONENTS),
           ...
           MIN_LOG_COMPONENTS,
           MAX_LOG_COMPONENTS);
  DevCheck((level < NUM_LOG_LEVEL) && (level >= OAILOG_DISABLE), level, NUM_LOG_LEVEL, OAILOG_ERR);
