                              ...
                              enum stream_status_e *stream_status)
{
  ...
  const openair0_timestamp writeTimestamp =
      timestamp + fp->get_samples_slot_timestamp(slot, fp, DURATION_RX_TO_TX) - UE->N_TA_offset - UE->old_timing_advance;

  // but use current UE->timing_advance value to compute writeBlockSize
  if (UE->timing_advance != UE->old_timing_advance) {
    writeBlockSize -= UE->timing_advance - UE->old_timing_advance;
    UE->old_timing_advance = UE->timing_advance;
  }
  ...
  if (proc->rx_slot_type == NR_UPLINK_SLOT) {
    ...
    for (int symbol = 0; symbol < NR_SYMBOLS_PER_SLOT; symbol++) {
      ...
      if (!UE->sl_mode) {
        ...
        *sampleShift = (computedSampleShift != INT_MAX) ? computedSampleShift : *sampleShift;
...
void *UE_thread(void *arg)
{
  ...
  while (!oai_exit) {
    ...
    slot_process(UE, &curMsg.proc, &shiftForNextFrame, iq_shift_to_apply, UE->tx_resume_ind_fifo, tx_wait_for_dlsch, &stream_status);
  }
