...
void *ru_thread(void *param)
{
  ...
  if (get_softmodem_params()->calibrated_radio) {
    double tx_power_at_0dBFS = ru->config.ssb_config.ss_pbch_power.value + gNB->tx_amp_backoff_dB;
    openair0_config_t config;
    config.rx_power_reference = gNB->rx_power_reference_dBm;
    ...
    LOG_I(PHY, "Selected TX power referece %f, RX power reference %f\n", config.tx_power_reference, config.rx_power_reference);
  }
