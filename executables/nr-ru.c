...
void init_RU_proc(RU_t *ru) {
  ru->proc = (RU_proc_t){.ru = ru, .instance_cnt_emulateRF = -1, .first_rx = 1, .first_tx = 1};
  pthread_mutex_init(&ru->proc.mutex_emulateRF, NULL);
  pthread_cond_init(&ru->proc.cond_emulateRF, NULL);

  if(emulate_rf)
    threadCreate(&ru->proc.pthread_emulateRF, emulatedRF_thread, (void *)&ru->proc, "emulateRF", -1, OAI_PRIORITY_RT);
