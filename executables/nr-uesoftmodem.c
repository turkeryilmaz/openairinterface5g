...
void init_openair0(PHY_VARS_NR_UE *ue)
{
  ...
  for (card=0; card<MAX_CARDS; card++) {
    ...
    cfg->threequarter_fs   = frame_parms->threequarter_fs;
    ...
    if (get_nrUE_params()->usrp_args) cfg->sdr_addrs = get_nrUE_params()->usrp_args;
    if (get_nrUE_params()->tx_subdev) cfg->tx_subdev = get_nrUE_params()->tx_subdev;
    if (get_nrUE_params()->rx_subdev) cfg->rx_subdev = get_nrUE_params()->rx_subdev;
