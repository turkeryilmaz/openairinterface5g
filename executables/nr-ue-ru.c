...
static volatile int firstTS_initialized = 0;
...
{
  if (UE->Mod_id != 0) // we have to wait until UE 0 performed the first initial read from all RUs
    while (!firstTS_initialized)
      usleep(1000);
  ...
  *ptimestamp = tmp_timestamp - dev0->firstTS;
  ...
  // and in the first run, it has to read from all RUs and initialize each firstTS
  ...
  for (int ru_id = 0; ru_id < nrue_ru_count; ru_id++) {
    ...
    if (firstTS_initialized && ue_id >= 0) // skip cells that are already used by an UE
      continue;
    if (ue_id == UE->Mod_id) // skip if this is our cell (we did already read above)
      continue;
