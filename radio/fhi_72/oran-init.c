                                  ...
                                  const struct xran_fh_config *fh_config)
{
  ...
  for (int i=0; i < xran_max_antenna_nr; i++) {
    ...
    // Set RU_PORT_ID and beam to i so that each stream would use different beam id.
    dlPm[i] = get_xran_prb_map(fh_config, XRAN_DIR_DL, 0, 14, i, i);
    ulPm[i] = get_xran_prb_map(fh_config, XRAN_DIR_UL, 0, 14, i, i);
