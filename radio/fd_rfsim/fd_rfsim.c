...
static void fh_if4p5_south_in(RU_t *ru, int *frame, int *slot)
{
  ...
  LOG_D(HW, "FHI_ADATPER: fh_if4p5_south_in: frame %d, slot %d\n", *frame, *slot);
  // O-DU expects
...
static bool configure_fd_rfsim(openair0_device *device, openair0_config_t *openair0_config)
{
  ...
  printf("FHI adapter loaded. %d\n", ru_lower->rfdevice.trx_start_func == NULL);
                                                              ...
                                                              eth_params_t *eth_params)
{
  ...
  AssertFatal(ret, "Failed to configure fhi adapter");
