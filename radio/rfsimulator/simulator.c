...
static int rfsimulator_set_gains(openair0_device *device, openair0_config_t *openair0_cfg) {
  const int rfsimulator_0dbFS_tx_power_at_0_gain = 0;
  const int rfsimulator_0dBFS_rx_power_at_0_gain = 0;
  rfsimulator_state_t *s = device->priv;
  s->tx_power_reference = rfsimulator_0dbFS_tx_power_at_0_gain + openair0_cfg->tx_gain[0];

  s->rx_power_reference = rfsimulator_0dBFS_rx_power_at_0_gain - openair0_cfg->rx_gain[0];
  return 0;
}
...
int device_init(openair0_device *device, openair0_config_t *openair0_cfg) {
  ...
  rfsimulator_set_tx_power_reference(device, openair0_cfg);
  rfsimulator_set_rx_power_reference(device, openair0_cfg);
