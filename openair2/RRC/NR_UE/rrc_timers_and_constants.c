...
void handle_meas_timers(NR_UE_RRC_INST_t *rrc)
{
  ...
  for (int i = 0; i < NB_CNX_UE; i++) {
    ...
    bool ta2_expired = nr_timer_tick(&timers->TA2);
    if (ta2_expired) {
      ...
      if (l3_measurements->reports_sent < l3_measurements->max_reports) {
        nr_timer_setup(&timers->periodic_report_timer, l3_measurements->report_interval_ms, 10);
        nr_timer_start(&timers->periodic_report_timer);
      }
    }

    bool periodic_expired = nr_timer_tick(&timers->periodic_report_timer);
