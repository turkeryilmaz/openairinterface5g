...
static void handle_meas_reporting_remove(rrcPerNB_t *rrc, int id, NR_UE_Timers_Constants_t *timers)
{
  ...
  nr_timer_stop(&timers->TA2);
  nr_timer_stop(&timers->periodic_report_timer);

  rrc->l3_measurements.reports_sent = 0;
  rrc->l3_measurements.max_reports = 0;
  rrc->l3_measurements.report_interval_ms = 0;
}
...
static void nr_ue_check_meas_report(NR_UE_RRC_INST_t *rrc, const uint8_t gnb_index)
{
  ...
  for (int i = 0; i < MAX_MEAS_CONFIG; i++) {
    ...
    if (serving_cell_rsrp + rsrp_hysteresis < rsrp_threshold) {
      if (!nr_timer_is_active(&timers->TA2) && (l3_measurements->reports_sent == 0)) {
        nr_timer_setup(&timers->TA2, get_A2_event_time_to_trigger(event_A2->timeToTrigger), 10);
        nr_timer_start(&timers->TA2);
    ...
    } else if (nr_timer_is_active(&timers->TA2) && (serving_cell_rsrp - rsrp_hysteresis > rsrp_threshold)) {
      nr_timer_stop(&timers->TA2);
      nr_timer_stop(&timers->periodic_report_timer);
      l3_measurements->reports_sent = 0;
    }
