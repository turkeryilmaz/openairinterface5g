...
static void nr_rrc_manage_rlc_bearers(const instance_t instance,
                                      ...
                                      rrcPerNB_t *rrc,
                                      const rnti_t rnti);
static int8_t nr_rrc_ue_process_meas_config(NR_MeasConfig_t *meas_config);

static void nr_rrc_ue_process_RadioBearerConfig(NR_UE_RRC_INST_t *ue_rrc,
                                                rnti_t rnti,
                                                const uint8_t gNB_index,
                                                NR_RadioBearerConfig_t *const radioBearerConfig);
static void nr_rrc_ue_generate_RRCSetupRequest(NR_UE_RRC_INST_t *rrc, const uint8_t gNB_index);
...
{
  NR_UE_RRC_SRB_INFO_t *Srb0 = &NR_UE_rrc_inst[Mod_idP].perNB[gNB_id].Srb[0];
  memcpy(output, Srb0->srb_buffers.Tx_buffer.Payload, Srb0->srb_buffers.Tx_buffer.payload_size);
...
// RRCReconfiguration
int8_t nr_rrc_ue_process_rrcReconfiguration(const instance_t instance, NR_RRCReconfiguration_t *rrcReconfiguration)
{
    ...
    case NR_RRCReconfiguration__criticalExtensions_PR_rrcReconfiguration:
      if(rrcReconfiguration->criticalExtensions.choice.rrcReconfiguration->radioBearerConfig != NULL){
        if (rrc->radio_bearer_config == NULL) {
          rrc->radio_bearer_config = rrcReconfiguration->criticalExtensions.choice.rrcReconfiguration->radioBearerConfig;
        } else {
      ...
      if(rrcReconfiguration->criticalExtensions.choice.rrcReconfiguration->secondaryCellGroup != NULL){
        ...
        if (get_softmodem_params()->sa || get_softmodem_params()->nsa)
          ...
          nr_rrc_manage_rlc_bearers(instance, cellGroupConfig, rrc->perNB + 0, rrc->rnti);
        ...
        if (get_softmodem_params()->sa || get_softmodem_params()->nsa) {
          ...
          nr_rrc_ue_decode_secondary_cellgroup_config(rrc, cellGroupConfig);
      }
      if(rrcReconfiguration->criticalExtensions.choice.rrcReconfiguration->measConfig != NULL){
        if (rrc->meas_config == NULL) {
          rrc->meas_config = rrcReconfiguration->criticalExtensions.choice.rrcReconfiguration->measConfig;
        } else {
    ...
    case nr_SecondaryCellGroupConfig_r15:
      {
        ...
        nr_rrc_ue_process_rrcReconfiguration(0, RRCReconfiguration);
    ...
    case nr_RadioBearerConfigX_r15:
      {
        ...
        if (get_softmodem_params()->nsa) {
          ...
          nr_rrc_ue_process_RadioBearerConfig(rrc, rrc->rnti, 0, RadioBearerConfig);
                                                 ...
                                                 const uint8_t buffer_len)
{
  ...
  // ASN_STRUCT_FREE(asn_DEF_NR_MIB, rrc->mib);
  // rrc->mib = bcch_message->message.choice.mib;
  ...
  /*
      ...
      itti_send_msg_to_task(TASK_MAC_UE, instance, msg);
...
void nr_rrc_ue_generate_ra_msg(instance_t instance, RA_trigger_t trigger, uint8_t gNB_index)
{
  switch (trigger) {
    case INITIAL_ACCESS_FROM_RRC_IDLE:
      ...
      nr_rrc_ue_generate_RRCSetupRequest(NR_UE_rrc_inst + instance, gNB_index);
      break;
  ...
  if (bcch_message->message.present == NR_BCCH_DL_SCH_MessageType_PR_c1) {
    switch (bcch_message->message.choice.c1->present) {
      case NR_BCCH_DL_SCH_MessageType__c1_PR_systemInformationBlockType1:
        ...
        nr_rrc_ue_generate_ra_msg(instance, INITIAL_ACCESS_FROM_RRC_IDLE, gNB_index);
        break;
                                              ...
                                              long *fullConfig)
{
  ...
  nr_rrc_manage_rlc_bearers(instance, cellGroupConfig, rrc->perNB + gNB_index, rnti);
  ...
  if(fullConfig)
   ...
   nr_rrc_set_sib1_timers_and_constants(&rrc->timers_and_constants, rrc->perNB[gNB_index].SInfo.sib1);
...
static void rrc_ue_generate_RRCSetupComplete(instance_t instance,
                                             rnti_t rnti,
                                             const uint8_t gNB_index,
                                             ...
                                             uint8_t sel_plmn_id)
{
  ...
  LOG_I(NR_RRC,
        "[UE %ld][RAPROC] Logical Channel UL-DCCH (SRB1), Generating RRCSetupComplete (bytes%d, gNB %d)\n",
        instance,
        size,
        gNB_index);
  int srb_id = 1; // RRC setup complete on SRB1
  LOG_D(NR_RRC,
        "[RRC_UE %ld][][--- PDCP_DATA_REQ/%d Bytes (RRCSetupComplete to gNB %d ) --->][PDCP][MOD %ld][RB "
        "%02d]\n",
        instance,
        size,
        gNB_index,
...
{
  NR_UE_RRC_INST_t *rrc = &NR_UE_rrc_inst[instance];
  NR_UE_RRC_SRB_INFO_t *srb0 = &rrc->perNB[ind->gnb_index].Srb[0];
  memcpy(srb0->srb_buffers.Rx_buffer.Payload, ind->sdu, ind->sdu_size);
  srb0->srb_buffers.Rx_buffer.payload_size = ind->sdu_size;
  ...
  LOG_D(RRC,
        ...
        instance,
        srb0->srb_buffers.Rx_buffer.payload_size,
        rrc->nrRrcState);
  dec_rval = uper_decode(NULL,
                         &asn_DEF_NR_DL_CCCH_Message,
                         (void **)&dl_ccch_msg,
                         (uint8_t *)srb0->srb_buffers.Rx_buffer.Payload,
                         srb0->srb_buffers.Rx_buffer.payload_size,
                         0,
                         0);
 ...
 // from NR SRB3
 int8_t nr_rrc_ue_decode_NR_DL_DCCH_Message(const instance_t instance,
                                            const uint8_t gNB_index,
                                            ...
                                            const uint32_t buffer_len)
 {
   ...
   if(nr_dl_dcch_msg != NULL){
	 ...
	 switch(nr_dl_dcch_msg->message.choice.c1->present){
       ...
       nr_rrc_ue_process_rrcReconfiguration(instance, nr_dl_dcch_msg->message.choice.c1->choice.rrcReconfiguration);
       break;
     ...
     case NR_DL_DCCH_MessageType__c1_PR_NOTHING:
	   ...
	   case NR_DL_DCCH_MessageType__c1_PR_rrcRelease:
		...
		(nr_dl_dcch_msg->message.choice.c1->present == NR_DL_DCCH_MessageType__c1_PR_rrcRelease)){
         ...
         itti_send_msg_to_task(TASK_RRC_NRUE, instance, msg_p);
         break;
 ...
 {
   uint8_t rv[6];
   NR_UE_RRC_SRB_INFO_t *Srb0 = &rrc->perNB[gNB_index].Srb[0];
   if (Srb0->srb_buffers.Tx_buffer.payload_size == 0) {
    ...
    Srb0->srb_buffers.Tx_buffer.payload_size =
        do_RRCSetupRequest((uint8_t *)Srb0->srb_buffers.Tx_buffer.Payload, sizeof(Srb0->srb_buffers.Tx_buffer.Payload), rv);
    LOG_I(NR_RRC,
          "Logical Channel UL-CCCH (SRB0), Generating RRCSetupRequest (bytes %d, gNB %d)\n",
          Srb0->srb_buffers.Tx_buffer.payload_size,
          gNB_index);
                                                ...
                                                NR_RadioBearerConfig_t *const radioBearerConfig)
{
  ...
  if (radioBearerConfig->srb_ToAddModList != NULL) {
    for (int cnt = 0; cnt < radioBearerConfig->srb_ToAddModList->list.count; cnt++) {
      ...
      NR_UE_RRC_SRB_INFO_t *Srb_info = &ue_rrc->perNB[gNB_index].Srb[srb->srb_Identity];
  ...
  if (radioBearerConfig->drb_ToAddModList != NULL) {
    for (int cnt = 0; cnt < radioBearerConfig->drb_ToAddModList->list.count; cnt++) {
      ...
      if (ue_rrc->perNB[gNB_index].active_DRBs[DRB_id]) {
        ...
        ue_rrc->perNB[gNB_index].active_DRBs[DRB_id] = true;
        add_drb(false,
                rnti,
  ...
  LOG_I(NR_RRC, "State = NR_RRC_CONNECTED (gNB %d)\n", gNB_index);
 }

 //-----------------------------------------------------------------------------
 static void rrc_ue_process_rrcReconfiguration(instance_t instance,
                                               rnti_t rnti,
                                               NR_RRCReconfiguration_t *rrcReconfiguration,
                                               uint8_t gNB_index)
 //-----------------------------------------------------------------------------
 {
   LOG_I(NR_RRC, "[UE %ld] Receiving from SRB1 (DL-DCCH), Processing RRCReconfiguration (gNB %d)\n", instance, gNB_index);
   ...
   if (rrcReconfiguration->criticalExtensions.present
     ...
     if (ie->measConfig != NULL) {
       ...
       nr_rrc_ue_process_measConfig(NR_UE_rrc_inst[instance].perNB + gNB_index, ie->measConfig);
     }
     ...
     if (ie->radioBearerConfig != NULL) {
       ...
       nr_rrc_ue_process_RadioBearerConfig(&NR_UE_rrc_inst[instance], rnti, gNB_index, ie->radioBearerConfig);
     }
 ...
 static void nr_rrc_ue_generate_RRCReconfigurationComplete(rnti_t rnti,
                                                           const uint8_t gNB_index,
                                                           const int srb_id,
                                                           const uint8_t Transaction_id)
 {
   ...
   LOG_D(RLC,
         "PDCP_DATA_REQ/%d Bytes (RRCReconfigurationComplete to gNB %d ) "
         "--->][PDCP][RB %02d]\n",
         size,
         gNB_index,
         srb_id);
...

 {
   ...
   if (dl_dcch_msg->message.present == NR_DL_DCCH_MessageType_PR_c1) {
     switch (dl_dcch_msg->message.choice.c1->present) {
       ...
       case NR_DL_DCCH_MessageType__c1_PR_rrcReconfiguration: {
         rrc_ue_process_rrcReconfiguration(instance, rnti, dl_dcch_msg->message.choice.c1->choice.rrcReconfiguration, gNB_indexP);
       ...
       case NR_DL_DCCH_MessageType__c1_PR_ueCapabilityEnquiry:
         ...
         nr_rrc_ue_process_ueCapabilityEnquiry(rrc, dl_dcch_msg->message.choice.c1->choice.ueCapabilityEnquiry);
         break;
       case NR_DL_DCCH_MessageType__c1_PR_rrcReestablishment:
         LOG_I(NR_RRC, "Logical Channel DL-DCCH (SRB1), Received RRCReestablishment\n");
         nr_rrc_ue_generate_rrcReestablishmentComplete(dl_dcch_msg->message.choice.c1->choice.rrcReestablishment);
         break;
       case NR_DL_DCCH_MessageType__c1_PR_dlInformationTransfer: {
         ...
         if (dlInformationTransfer->criticalExtensions.present
           ...
           NAS_DOWNLINK_DATA_IND(msg_p).UEid = instance; // TODO set the UEid to something else ?
       ...
       case NR_DL_DCCH_MessageType__c1_PR_securityModeCommand:
         ...
         nr_rrc_ue_process_securityModeCommand(rrc, dl_dcch_msg->message.choice.c1->choice.securityModeCommand, rnti, gNB_indexP);
         break;
     }
   }
   return 0;
  ...
  while (1) {
    ...
    switch (ITTI_MSG_ID(msg_p)) {
      ...
      case NRRRC_FRAME_PROCESS:
        LOG_D(NR_RRC, "[UE %ld] Received %s: frame %d\n", instance, ITTI_MSG_NAME(msg_p), NRRRC_FRAME_PROCESS(msg_p).frame);
      ...
      case NR_RRC_MAC_CCCH_DATA_IND: {
        ...
        nr_rrc_ue_decode_ccch(instance, ind->rnti, ind, ind->gnb_index);
      } break;
