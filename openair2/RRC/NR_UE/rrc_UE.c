...
void nr_rrc_ue_generate_rrcReestablishmentComplete(const protocol_ctxt_t *const ctxt_pP,
...
{
  AssertFatal((RRC_STATE_FIRST_NR <= state) && (state <= RRC_STATE_LAST_NR), "Invalid state %d!\n", state);
    ...
    switch (NR_UE_rrc_inst[ue_mod_idP].nrRrcState) {
      case RRC_STATE_INACTIVE_NR:
        AssertFatal((RRC_SUB_STATE_INACTIVE_FIRST_NR <= subState) && (subState <= RRC_SUB_STATE_INACTIVE_LAST_NR),
                    ...
                    subState,
                    NR_UE_rrc_inst[ue_mod_idP].nrRrcState);
        break;
...
int8_t nr_rrc_ue_decode_secondary_cellgroup_config(const module_id_t module_id, const uint8_t *buffer, const uint32_t size)
{
  ...
  asn_dec_rval_t dec_rval =
      uper_decode(NULL, &asn_DEF_NR_CellGroupConfig, (void **)&cell_group_config, (uint8_t *)buffer, size, 0, 0);
    ...
    case NR_RRCReconfiguration__criticalExtensions_PR_rrcReconfiguration:
      if (rrcReconfiguration->criticalExtensions.choice.rrcReconfiguration->radioBearerConfig != NULL) {
        if (NR_UE_rrc_inst[module_id].radio_bearer_config == NULL) {
          NR_UE_rrc_inst[module_id].radio_bearer_config =
              rrcReconfiguration->criticalExtensions.choice.rrcReconfiguration->radioBearerConfig;
        } else {
          if (LOG_DEBUGFLAG(DEBUG_ASN1)) {
            struct NR_RadioBearerConfig *RadioBearerConfig =
                rrcReconfiguration->criticalExtensions.choice.rrcReconfiguration->radioBearerConfig;
...
NR_UE_RRC_INST_t *openair_rrc_top_init_ue_nr(char *uecap_file, char *rrc_config_path)
{
  if (NB_NR_UE_INST > 0) {
    ...
    if (get_softmodem_params()->phy_test == 1 || get_softmodem_params()->do_ra == 1) {
      ...
      fd = fopen(filename, "r");
      char buffer[1024];
      AssertFatal(fd, "cannot read file %s: errno %d, %s\n", filename, errno, strerror(errno));
...
int8_t nr_rrc_ue_decode_NR_BCCH_BCH_Message(const module_id_t module_id,
                                            const uint8_t gNB_index,
    ...
    switch (typeandinfo->present) {
      case NR_SystemInformation_IEs__sib_TypeAndInfo__Member_PR_sib2:
        if ((SI_info->SIStatus & 2) == 0) {
          ...
          LOG_I(RRC,
                "[UE %" PRIu8 "] Frame %" PRIu32 " Found SIB2 from gNB %" PRIu8 "\n",
                ctxt_pP->module_id,
                ctxt_pP->frame,
                gNB_index);
          nr_dump_sib2(SI_info->sib2);
          LOG_I(RRC,
                "[FRAME %05" PRIu32 "][RRC_UE][MOD %02" PRIu8 "][][--- MAC_CONFIG_REQ (SIB2 params  gNB %" PRIu8
                ") --->][MAC_UE][MOD %02" PRIu8 "][]\n",
                ctxt_pP->frame,
                ctxt_pP->module_id,
                gNB_index,
      ...
      case NR_SystemInformation_IEs__sib_TypeAndInfo__Member_PR_sib3:
        if ((SI_info->SIStatus & 4) == 0) {
          ...
          LOG_I(RRC,
                "[UE %" PRIu8 "] Frame %" PRIu32 " Found SIB3 from gNB %" PRIu8 "\n",
                ctxt_pP->module_id,
                ctxt_pP->frame,
                gNB_index);
      ...
      case NR_SystemInformation_IEs__sib_TypeAndInfo__Member_PR_sib4:
        if ((SI_info->SIStatus & 8) == 0) {
          ...
          LOG_I(RRC,
                "[UE %" PRIu8 "] Frame %" PRIu32 " Found SIB4 from gNB %" PRIu8 "\n",
                ctxt_pP->module_id,
                ctxt_pP->frame,
                gNB_index);
        }
      ...
      case NR_SystemInformation_IEs__sib_TypeAndInfo__Member_PR_sib5:
        if ((SI_info->SIStatus & 16) == 0) {
          ...
          LOG_I(RRC,
                "[UE %" PRIu8 "] Frame %" PRIu32 " Found SIB5 from gNB %" PRIu8 "\n",
                ctxt_pP->module_id,
                ctxt_pP->frame,
                gNB_index);
        }
      ...
      case NR_SystemInformation_IEs__sib_TypeAndInfo__Member_PR_sib6:
        if ((SI_info->SIStatus & 32) == 0) {
          ...
          LOG_I(RRC,
                "[UE %" PRIu8 "] Frame %" PRIu32 " Found SIB6 from gNB %" PRIu8 "\n",
                ctxt_pP->module_id,
                ctxt_pP->frame,
                gNB_index);
        }
      ...
      case NR_SystemInformation_IEs__sib_TypeAndInfo__Member_PR_sib7:
        if ((SI_info->SIStatus & 64) == 0) {
          ...
          LOG_I(RRC,
                "[UE %" PRIu8 "] Frame %" PRIu32 " Found SIB7 from gNB %" PRIu8 "\n",
                ctxt_pP->module_id,
                ctxt_pP->frame,
                gNB_index);
        }
      ...
      case NR_SystemInformation_IEs__sib_TypeAndInfo__Member_PR_sib8:
        if ((SI_info->SIStatus & 128) == 0) {
          ...
          LOG_I(RRC,
                "[UE %" PRIu8 "] Frame %" PRIu32 " Found SIB8 from gNB %" PRIu8 "\n",
                ctxt_pP->module_id,
                ctxt_pP->frame,
                gNB_index);
        }
      ...
      case NR_SystemInformation_IEs__sib_TypeAndInfo__Member_PR_sib9:
        if ((SI_info->SIStatus & 256) == 0) {
          ...
          LOG_I(RRC,
                "[UE %" PRIu8 "] Frame %" PRIu32 " Found SIB9 from gNB %" PRIu8 "\n",
                ctxt_pP->module_id,
                ctxt_pP->frame,
                gNB_index);
        }
      ...
      case NR_SystemInformation_IEs__sib_TypeAndInfo__Member_PR_sib10_v1610:
        if ((SI_info->SIStatus & 512) == 0) {
          ...
          LOG_I(RRC,
                "[UE %" PRIu8 "] Frame %" PRIu32 " Found SIB10 from gNB %" PRIu8 "\n",
                ctxt_pP->module_id,
                ctxt_pP->frame,
                gNB_index);
        }
      ...
      case NR_SystemInformation_IEs__sib_TypeAndInfo__Member_PR_sib11_v1610:
        if ((SI_info->SIStatus & 1024) == 0) {
          ...
          LOG_I(RRC,
                "[UE %" PRIu8 "] Frame %" PRIu32 " Found SIB11 from gNB %" PRIu8 "\n",
                ctxt_pP->module_id,
                ctxt_pP->frame,
                gNB_index);
        }
      ...
      case NR_SystemInformation_IEs__sib_TypeAndInfo__Member_PR_sib12_v1610:
        if ((SI_info->SIStatus & 2048) == 0) {
          ...
          LOG_I(RRC,
                "[UE %" PRIu8 "] Frame %" PRIu32 " Found SIB12 from gNB %" PRIu8 "\n",
                ctxt_pP->module_id,
                ctxt_pP->frame,
                gNB_index);
        }
      ...
      case NR_SystemInformation_IEs__sib_TypeAndInfo__Member_PR_sib13_v1610:
        if ((SI_info->SIStatus & 4096) == 0) {
          ...
          LOG_I(RRC,
                "[UE %" PRIu8 "] Frame %" PRIu32 " Found SIB13 from gNB %" PRIu8 "\n",
                ctxt_pP->module_id,
                ctxt_pP->frame,
                gNB_index);
          ...
          LOG_I(RRC,
                "[FRAME %05" PRIu32 "][RRC_UE][MOD %02" PRIu8 "][][--- MAC_CONFIG_REQ (SIB13 params gNB %" PRIu8
                ") --->][MAC_UE][MOD %02" PRIu8 "][]\n",
                ctxt_pP->frame,
                ctxt_pP->module_id,
                gNB_index,
      ...
      case NR_SystemInformation_IEs__sib_TypeAndInfo__Member_PR_sib14_v1610:
        if ((SI_info->SIStatus & 8192) == 0) {
          ...
          LOG_I(RRC,
                "[UE %" PRIu8 "] Frame %" PRIu32 " Found SIB14 from gNB %" PRIu8 "\n",
                ctxt_pP->module_id,
                ctxt_pP->frame,
                gNB_index);
        }
                                       ...
                                       long *fullConfig)
{
  ...
  if (cellGroupConfig->mac_CellGroupConfig != NULL) {
    ...
    memcpy(rrc->cell_group_config->mac_CellGroupConfig,
           cellGroupConfig->mac_CellGroupConfig,
  ...
  if (cellGroupConfig->spCellConfig != NULL) {
    ...
    if (cellGroupConfig->spCellConfig->reconfigurationWithSync != NULL) {
      ...
      // TODO: Implementation of the remaining procedures regarding the reception of the reconfigurationWithSync, TS 38.331 -
      // Section 5.3.5.5.2
    }

    if (rrc->cell_group_config && rrc->cell_group_config->spCellConfig) {
      memcpy(rrc->cell_group_config->spCellConfig, cellGroupConfig->spCellConfig, sizeof(struct NR_SpCellConfig));
    } else {
...
void configure_spcell(NR_UE_RRC_INST_t *rrc, NR_SpCellConfig_t *spcell_config)
{
  ...
  if (spcell_config->spCellConfigDedicated) {
    ...
    if (scd->firstActiveDownlinkBWP_Id) {
      ...
      // if any of the reference signal(s) that are used for radio link monitoring are reconfigured by the received
      // spCellConfigDedicated reset RLF timers and constants
                                             ...
                                             uint8_t sel_plmn_id)
{
  ...
  size = do_RRCSetupComplete(ctxt_pP->module_id, buffer, sizeof(buffer), Transaction_id, sel_plmn_id, nas_msg_length, nas_msg);
  LOG_I(NR_RRC,
    ...
    if (NR_UE_rrc_inst[ctxt_pP->module_id].SInfo[gNB_index].SIStatus > 0) {
      switch (dl_ccch_msg->message.choice.c1->present) {
        ...
        case NR_DL_CCCH_MessageType__c1_PR_rrcSetup:
          ...
          rrc_ue_generate_RRCSetupComplete(ctxt_pP,
                                           gNB_index,
...
// from NR SRB3
int8_t nr_rrc_ue_decode_NR_DL_DCCH_Message(const module_id_t module_id,
                                           const uint8_t gNB_index,
                                           ...
                                           const uint32_t buffer_len)
{
  ...
  asn_dec_rval_t dec_rval =
      uper_decode(NULL, &asn_DEF_NR_DL_DCCH_Message, (void **)&nr_dl_dcch_msg, (uint8_t *)bufferP, buffer_len, 0, 0);
      ...
      case NR_DL_DCCH_MessageType_PR_c1:
        switch (nr_dl_dcch_msg->message.choice.c1->present) {
          ...
          case NR_DL_DCCH_MessageType__c1_PR_rrcRelease:
            ...
            if ((nr_dl_dcch_msg->message.choice.c1->choice.rrcRelease->criticalExtensions.present
              ...
              nr_dl_dcch_msg->message.choice.c1->choice.rrcRelease->criticalExtensions.choice.rrcRelease->deprioritisationReq
                                           ...
                                           const uint8_t gNB_index)
{
  ...
  if (securityModeCommand->criticalExtensions.present == NR_SecurityModeCommand__criticalExtensions_PR_securityModeCommand) {
    ...
    ul_dcch_msg.message.choice.c1->choice.securityModeComplete->rrc_TransactionIdentifier =
    ...
    ul_dcch_msg.message.choice.c1->choice.securityModeComplete->criticalExtensions.choice.securityModeComplete =
        CALLOC(1, sizeof(NR_SecurityModeComplete_IEs_t));
    ul_dcch_msg.message.choice.c1->choice.securityModeComplete->criticalExtensions.choice.securityModeComplete
        ->nonCriticalExtension = NULL;
    LOG_I(NR_RRC,
          "[UE %d] SFN/SF %d/%d: Receiving from SRB1 (DL-DCCH), encoding securityModeComplete (gNB %d), rrc_TransactionIdentifier: "
          "%ld\n",
          ctxt_pP->module_id,
          ctxt_pP->frame,
          ctxt_pP->subframe,
...
//-----------------------------------------------------------------------------
int32_t nr_rrc_ue_establish_srb1(module_id_t ue_mod_idP, frame_t frameP, uint8_t gNB_index, NR_SRB_ToAddMod_t *SRB_config)
//-----------------------------------------------------------------------------
{
...
int32_t nr_rrc_ue_establish_srb2(module_id_t ue_mod_idP, frame_t frameP, uint8_t gNB_index, NR_SRB_ToAddMod_t *SRB_config)
{
  // add descriptor from RRC PDU
...
int32_t nr_rrc_ue_establish_drb(module_id_t ue_mod_idP, frame_t frameP, uint8_t gNB_index, NR_DRB_ToAddMod_t *DRB_config)
{
  // add descriptor from RRC PDU
...
//-----------------------------------------------------------------------------
void nr_rrc_ue_process_measConfig(const protocol_ctxt_t *const ctxt_pP, const uint8_t gNB_index, NR_MeasConfig_t *const measConfig)
//-----------------------------------------------------------------------------
{
  int i;
  long ind;
      ...
      if (SRB_id == 1) {
        if (NR_UE_rrc_inst[ctxt_pP->module_id].SRB1_config[gNB_index]) {
          ...
          nr_rrc_mac_config_req_ue_logicalChannelBearer(ctxt_pP->module_id,
                                                        ...
                                                        gNB_index,
                                                        1,
                                                        true); // todo handle mac_LogicalChannelConfig
          // rrc_mac_config_req_ue
        }
      } else {
        ...
        if (rlc_bearer2add_list != NULL) {
          for (j = 0; j < rlc_bearer2add_list->list.count; j++) {
            if (rlc_bearer2add_list->list.array[j]->servedRadioBearer != NULL) {
              if (rlc_bearer2add_list->list.array[j]->servedRadioBearer->present
                ...
                if (DRB_id == rlc_bearer2add_list->list.array[j]->servedRadioBearer->choice.drb_Identity) {
                  LOG_I(NR_RRC,
                        ...
                        ctxt_pP->frame,
                        ctxt_pP->module_id,
                        rlc_bearer2add_list->list.array[j]->logicalChannelIdentity,
                        0,
                        ctxt_pP->module_id);
                  nr_rrc_mac_config_req_ue_logicalChannelBearer(ctxt_pP->module_id,
                                                                0,
  ...
  if (NR_UE_rrc_inst[ctxt_pP->module_id].cell_group_config->rlc_BearerToReleaseList != NULL) {
    for (i = 0; i < NR_UE_rrc_inst[ctxt_pP->module_id].cell_group_config->rlc_BearerToReleaseList->list.count; i++) {
      NR_LogicalChannelIdentity_t lcid =
          *NR_UE_rrc_inst[ctxt_pP->module_id].cell_group_config->rlc_BearerToReleaseList->list.array[i];
      LOG_I(NR_RRC,
    ...
    switch (ITTI_MSG_ID(msg_p)) {
      ...
      case NR_RRC_MAC_BCCH_DATA_IND:
        LOG_D(NR_RRC,
              ...
              ue_mod_id,
              ITTI_MSG_NAME(msg_p),
              NR_RRC_MAC_BCCH_DATA_IND(msg_p).frame,
      ...
      case NR_RRC_MAC_CCCH_DATA_IND:
        ...
        memcpy(srb_info_p->Rx_buffer.Payload, NR_RRC_MAC_CCCH_DATA_IND(msg_p).sdu, NR_RRC_MAC_CCCH_DATA_IND(msg_p).sdu_size);
        ...
        PROTOCOL_CTXT_SET_BY_INSTANCE(&ctxt,
                                      ...
                                      NR_RRC_MAC_CCCH_DATA_IND(msg_p).frame,
                                      0);
        // PROTOCOL_CTXT_SET_BY_MODULE_ID(&ctxt, ue_mod_id, GNB_FLAG_NO, NR_RRC_MAC_CCCH_DATA_IND (msg_p).rnti,
        // NR_RRC_MAC_CCCH_DATA_IND (msg_p).frame, 0, NR_RRC_MAC_CCCH_DATA_IND (msg_p).gnb_index);
        nr_rrc_ue_decode_ccch(&ctxt, srb_info_p, NR_RRC_MAC_CCCH_DATA_IND(msg_p).gnb_index);
        break;
...
void nr_rrc_ue_process_sidelink_radioResourceConfig(module_id_t Mod_idP,
                                                    uint8_t gNB_index,
...
//-----------------------------------------------------------------------------
{
  ...
  ul_dcch_msg.message.choice.c1->choice.ueCapabilityInformation->rrc_TransactionIdentifier =
      UECapabilityEnquiry->rrc_TransactionIdentifier;
  ...
  if (get_softmodem_params()->nsa == 1) {
    OCTET_STRING_t *requestedFreqBandsNR =
        UECapabilityEnquiry->criticalExtensions.choice.ueCapabilityEnquiry->ue_CapabilityEnquiryExt;
  ...
  ul_dcch_msg.message.choice.c1->choice.ueCapabilityInformation->criticalExtensions.choice.ueCapabilityInformation =
      CALLOC(1, sizeof(struct NR_UECapabilityInformation_IEs));
  ul_dcch_msg.message.choice.c1->choice.ueCapabilityInformation->criticalExtensions.choice.ueCapabilityInformation
      ->ue_CapabilityRAT_ContainerList = CALLOC(1, sizeof(struct NR_UE_CapabilityRAT_ContainerList));
  ul_dcch_msg.message.choice.c1->choice.ueCapabilityInformation->criticalExtensions.choice.ueCapabilityInformation
      ->ue_CapabilityRAT_ContainerList->list.count = 0;

  for (i = 0; i < UECapabilityEnquiry->criticalExtensions.choice.ueCapabilityEnquiry->ue_CapabilityRAT_RequestList.list.count;
       i++) {
    if (UECapabilityEnquiry->criticalExtensions.choice.ueCapabilityEnquiry->ue_CapabilityRAT_RequestList.list.array[i]->rat_Type
        == NR_RAT_Type_nr) {
      asn1cSeqAdd(&ul_dcch_msg.message.choice.c1->choice.ueCapabilityInformation->criticalExtensions.choice.ueCapabilityInformation
...
//-----------------------------------------------------------------------------
{
  ...
  int size = do_RRCReestablishmentComplete(buffer, RRC_BUFFER_SIZE, rrcReestablishment->rrc_TransactionIdentifier);
  LOG_I(NR_RRC,
...
void process_lte_nsa_msg(nsa_msg_t *msg, int msg_len)
{
  ...
  switch (msg_type) {
    case UE_CAPABILITY_ENQUIRY: {
      ...
      asn_dec_rval_t dec_rval =
          uper_decode_complete(NULL, &asn_DEF_NR_FreqBandList, (void **)&nr_freq_band_list, msg_buffer, msg_len);
      if ((dec_rval.code != RC_OK) && (dec_rval.consumed == 0)) {
    ...
    case RRC_MEASUREMENT_PROCEDURE: {
      ...
      asn_dec_rval_t dec_rval =
          uper_decode_complete(NULL, &asn_DEF_NR_MeasObjectToAddMod, (void **)&nr_meas_obj, msg_buffer, msg_len);
      if ((dec_rval.code != RC_OK) && (dec_rval.consumed == 0)) {
      ...
      LOG_D(NR_RRC,
            "NR carrierFreq_r15 (ssb): %ld and sub carrier spacing:%ld\n",
    ...
    case RRC_CONFIG_COMPLETE_REQ: {
      ...
      process_nsa_message(NR_UE_rrc_inst,
