                     ...
                     const uint8_t sdu_len)
{
  ...
  struct NR_MasterInformationBlockSidelink *mib_sl_rrc_buf = &NR_UE_rrc_inst[ctxt_pP->module_id].mib_sl[0]->
                                                              choice.c1->choice.masterInformationBlockSidelink;

  LOG_D(NR_RRC, "Decoded MIBSL SFN.SLOT %d.%d, InCoverage %d\n",
        mib_sl_buf->directFrameNumber_r16.buf, mib_sl_buf->slotIndex_r16.buf,
        (int)mib_sl_rrc_buf->inCoverage_r16);

  nr_rrc_mac_config_req_ue_sl(ctxt_pP->module_id, 0, 0, 0, 0, NULL,
