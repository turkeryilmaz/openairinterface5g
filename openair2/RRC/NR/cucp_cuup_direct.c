                                  ...
                                  instance_t instance)
{
  ...
  create_tunnel_req.ue_id = UE->gNB_ue_ngap_id;
...
static void cucp_cuup_bearer_context_setup_direct(e1ap_bearer_setup_req_t *const req, instance_t instance)
{
  ...
  PROTOCOL_CTXT_SET_BY_MODULE_ID(&ctxt, 0, GNB_FLAG_YES, UE->gNB_ue_ngap_id, 0, 0, 0);
  ...
  if(!NODE_IS_CU(RC.nrrrc[ctxt.module_id]->node_type)) {
    ...
    // GTP tunnel for DL
    fill_e1ap_bearer_setup_resp(&resp, req, gtpInst, UE->gNB_ue_ngap_id, remote_port, my_addr);
  }
