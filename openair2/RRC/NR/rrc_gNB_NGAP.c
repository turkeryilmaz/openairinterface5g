...
static DRB_nGRAN_to_setup_t fill_drb_ngran_tosetup(const drb_t *rrc_drb, const pdusession_t *session, const gNB_RRC_INST *rrc, const nr_redcap_ue_cap_t *redcap_cap)
{
...
bool trigger_bearer_setup(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE, int n, pdusession_t *sessions, uint64_t ueAggMaxBitRateDownlink)
{
  ...
  for (int i = 0; i < n; i++) {
    rrc_pdu_session_param_t *pduSession = add_pduSession(&UE->pduSessions, UE->rrc_ue_id, &sessions[i]);
    if (!pduSession) {
    ...
    for (int j = 0; j < pdu->numDRB2Setup; j++) {
      ...
      pdu->DRBnGRanList[0] = fill_drb_ngran_tosetup(rrc_drb, session, rrc, UE->redcap_cap);
    }
  }
...
int rrc_gNB_send_NGAP_PDUSESSION_MODIFY_RESP(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE, uint8_t xid)
{
  ...
  FOR_EACH_SEQ_ARR(rrc_pdu_session_param_t *, session, &UE->pduSessions) {
    ...
    if (session->status == PDU_SESSION_STATUS_DONE) {
      rrc_pdu_session_param_t *pduSession = find_pduSession(&UE->pduSessions, session->param.pdusession_id);
      if (pduSession) {
...
void rrc_gNB_send_NGAP_UE_CONTEXT_RELEASE_COMPLETE(instance_t instance, uint32_t gNB_ue_ngap_id, seq_arr_t *pdusessions)
{
  ...
  if (pdusessions && seq_arr_size(pdusessions)) {
    complete->num_pdu_sessions = seq_arr_size(pdusessions);
    FOR_EACH_SEQ_ARR(rrc_pdu_session_param_t *, session, pdusessions) {
      for (int i = 0; i < complete->num_pdu_sessions; ++i)
        complete->pdu_session_id[i] = session->param.pdusession_id;
    }
  }
  ...
  FOR_EACH_SEQ_ARR(rrc_pdu_session_param_t *, session, &UE->pduSessions) {
    if (xid == session->xid) {
      ...
      if (session->status == PDU_SESSION_STATUS_TORELEASE) {
        ...
        resp->pdusessions_failed[resp->nb_of_pdusessions_failed++].pdusession_id = pdusession->pdusession_id;
      }
