...
{
  const pdusession_t *elem = &((const rrc_pdu_session_param_t *)ptr)->param;
  free_byte_array(elem->nas_pdu);
}
...
drb_t *nr_rrc_add_drb(seq_arr_t *drb_ptr, int pdusession_id)
{
  ...
  LOG_I(NR_RRC, "Add DRB ID %d to established list (PDU Session ID=%d, total DRBs = %ld)\n", out->drb_id, pdusession_id, seq_arr_size(drb_ptr));
                   ...
                   const nr_pdcp_configuration_t *pdcp_config)
{
  ...
  DevAssert(rrc_drb != NULL); /* to double check that we create the same which we would retrieve */
  return rrc_drb;
}
