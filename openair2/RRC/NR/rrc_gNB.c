...
void nr_rrc_transfer_protected_rrc_message(const gNB_RRC_INST *rrc, const gNB_RRC_UE_t *ue_p, uint8_t srb_id, const uint8_t* buffer, int size)
{
  ...
  f1ap_dl_rrc_message_t dl_rrc = {.gNB_CU_ue_id = ue_p->gNB_ue_ngap_id, .gNB_DU_ue_id = ue_data->secondary_ue, .srb_id = srb_id};
  deliver_dl_rrc_message_data_t data = {.rrc = rrc, .dl_rrc = &dl_rrc};
  nr_pdcp_data_req_srb(ue_p->gNB_ue_ngap_id, srb_id, rrc_gNB_mui++, size, (unsigned char *const)buffer, rrc_deliver_dl_rrc_message, &data);
}
...
//-----------------------------------------------------------------------------
{
  ...
  LOG_DUMPMSG(NR_RRC, DEBUG_RRC,
              ...
              size,
              "[MSG] RRC Setup\n");
  nr_pdcp_add_srbs(true, ue_p->gNB_ue_ngap_id, SRBs, 0, NULL, NULL);
...
//-----------------------------------------------------------------------------
{
  ...
  nr_pdcp_add_srbs(ctxt_pP->enb_flag, ue_p->gNB_ue_ngap_id, SRBs, 0, NULL, NULL);
  ...
  f1ap_dl_rrc_message_t dl_rrc = {.gNB_CU_ue_id = ue_p->gNB_ue_ngap_id,
                                  .gNB_DU_ue_id = ue_data->secondary_ue,
                                  .srb_id = DCCH,
                                  .old_gNB_DU_ue_id = &old_gNB_DU_ue_id};
  deliver_dl_rrc_message_data_t data = {.rrc = rrc, .dl_rrc = &dl_rrc};
  nr_pdcp_data_req_srb(ue_p->gNB_ue_ngap_id, DCCH, rrc_gNB_mui++, size, (unsigned char *const)buffer, rrc_deliver_dl_rrc_message, &data);
}
  ...
  if (ul_ccch_msg->message.present == NR_UL_CCCH_MessageType_PR_c1) {
    switch (ul_ccch_msg->message.choice.c1->present) {
      ...
      case NR_UL_CCCH_MessageType__c1_PR_rrcReestablishmentRequest: {
        ...
        UE->rnti = rnti;
        cu_remove_f1_ue_data(UE->gNB_ue_ngap_id);
        ...
        UE->Srb[2].Active = 1;
        protocol_ctxt_t ctxt = {.rntiMaybeUEid = ue_context_p->ue_context.gNB_ue_ngap_id,
                                .module_id = module_id,
                                .instance = module_id,
                                .enb_flag = 1,
                                .eNB_index = module_id};
                                              ...
                                              const NR_RRCReconfigurationComplete_t *reconfig_complete)
{
  ...
  f1ap_ue_context_modif_req_t ue_context_modif_req = {
    .gNB_CU_ue_id = UE->gNB_ue_ngap_id,
    .gNB_DU_ue_id = ue_data->secondary_ue,
...
static void rrc_CU_process_ue_context_release_complete(MessageDef *msg_p)
{
  ...
  nr_pdcp_remove_UE(complete->gNB_CU_ue_id);
  newGtpuDeleteAllTunnels(instance, complete->gNB_CU_ue_id);
  rrc_gNB_send_NGAP_UE_CONTEXT_RELEASE_COMPLETE(instance, UE->gNB_ue_ngap_id);
  LOG_I(NR_RRC, "removed UE CU UE ID %u/RNTI %04x \n", complete->gNB_CU_ue_id, UE->rnti);
...
static void write_rrc_stats(const gNB_RRC_INST *rrc)
{
  ...
  RB_FOREACH(ue_context_p, rrc_nr_ue_tree_s, &((gNB_RRC_INST *)rrc)->rrc_ue_head)
  {
    ...
    const f1_ue_data_t *ue_data = cu_get_f1_ue_data(ue_ctxt->gNB_ue_ngap_id);
    DevAssert(ue_data != NULL);
    uint32_t du_ue_id = ue_data->secondary_ue;

    fprintf(f,
            "UE %d CU UE ID %d DU UE ID %d RNTI %04x random identity %016lx",
            i,
            ue_ctxt->gNB_ue_ngap_id,
            du_ue_id,
            ue_ctxt->rnti,
