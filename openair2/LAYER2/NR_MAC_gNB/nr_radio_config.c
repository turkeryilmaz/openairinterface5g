                                           ...
                                           int new_bwp)
{
  ...
  *uplinkConfig->firstActiveUplinkBWP_Id = new_bwp;
  ...
  if (old_bwp > 0) {   // we need to remove old BWP only if it is not the initial
    ...
    if (configDedicated->downlinkBWP_ToReleaseList->list.count == 0) {
      asn1cSequenceAdd(configDedicated->downlinkBWP_ToReleaseList->list, NR_BWP_Id_t, rel_id);
      *rel_id = id;
    } else
      *configDedicated->downlinkBWP_ToReleaseList->list.array[0] = id;
    ...
    if (uplinkConfig->uplinkBWP_ToReleaseList->list.count == 0) {
      asn1cSequenceAdd(uplinkConfig->uplinkBWP_ToReleaseList->list, NR_BWP_Id_t, rel_id);
      *rel_id = id;
    } else
      *uplinkConfig->uplinkBWP_ToReleaseList->list.array[0] = id;
  }
