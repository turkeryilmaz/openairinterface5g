...
void nr_rrc_config_ul_tda(NR_ServingCellConfigCommon_t *scc, int min_fb_delay, int do_SRS)
{
  ...
  if (scc->tdd_UL_DL_ConfigurationCommon) {
    ...
    int N_dl2 = p2->nrofDownlinkSlots;
    ...
    if (ul_symb >= mixed_min_ul_symb && (k2 <= N_dl1 || (p2 && N_ul2 == 0))) {
      ...
      asn1cSeqAdd(&tda_list->list, tda);
    } else if (ul_symb >= mixed_min_ul_symb && !p2 && k2 > N_dl1) { // k2 > N_Dl1
      ...
      asn1cSeqAdd(&tda_list->list, tda);
    } else if (ul_symb >= mixed_min_ul_symb && N_ul2 > 0 && k2 > N_dl1) {
    ...
    if (nb_slots_per_period % k2_msg3 == 0 /* see example above */
        && k2_msg3 > N_dl /* if less, can always have a D that reaches U */
        && k2_msg3 > N_ul /* if less, can always have a D that reaches U */
        && N_ul > k2 /* otherwise, below case is already covered */
        && no_mixed /* if mixed slot, even for DDDSU would reach */) {
      /* reach next UL from mixed slot in previous period */
      tda = set_TimeDomainResourceAllocation(k2 + 1, get_SLIV(0, 13));
      asn1cSeqAdd(&tda_list->list, tda);
    }
  }
