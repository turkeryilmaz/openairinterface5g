  ...
  while (max_dci > 0) {
    ...
    *next = fs_get_max(fs, *next, min_next);
    ...
    if (n_tda == 0) /* no TDA fulfills this */
      break;
    int start = 0;
    tda_info = get_best_ul_tda(nr_mac, beam, tda_info, n_tda, next->f, next->s, &start, &bw);
    DevAssert(tda_info->valid_tda);
    ...
    LOG_I(NR_MAC, "run pf_ul() at %4d.%2d with tda %d k2 %d (ULSCH at %4d.%2d) scheduled %d last_dl %d\n", frame, slot, tda, k2, next->f, next->s, sched, last_dl);
