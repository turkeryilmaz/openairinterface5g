...
static void positioning_information_response(const f1ap_positioning_information_resp_t *resp) {
    ...
    LOG_I(MAC, "Prepring PositioningInformationResponse gNB_CU_ue_id=%d, gNB_DU_ue_id=%d, ue_rnti= %04x\n", resp->gNB_CU_ue_id, resp->gNB_DU_ue_id, resp->nrppa_msg_info.ue_rnti);
    ...
    *f1ap_msg = *resp;
    ...
    UE_iterator(UE_info->list, UE) {
        if(UE->rnti==resp->nrppa_msg_info.ue_rnti) { // configuration details of specific UE // TODO manage non UE associated
            LOG_I(MAC,"Extracting SRS Configuration for Positioning_information_response for ue rnti= %04x \n", resp->nrppa_msg_info.ue_rnti); ////uid_t uid = &UE->uid;
            ...
            f1ap_msg->srs_configuration.srs_carrier_list.srs_carrier_list_item= malloc(maxnoSRScarrier*sizeof(f1ap_srs_carrier_list_item_t));
            ...
            LOG_D(MAC, "Preparing srs_carrier_list for NRPPA maxnoSRScarrier= %d", f1ap_msg->srs_configuration.srs_carrier_list.srs_carrier_list_length);
            for (int i = 0; i < maxnoSRScarrier; i++) {
                ...
                srs_carrier_list_item->active_ul_bwp.sRSConfig.sRSResource_List.srs_resource= malloc(maxnoSRSResources*sizeof(f1ap_srs_resource_t)); // TODO check null condition
                ...
                for(int k=0; k < maxnoSRSResources; k++) { //Preparing SRS Resource List
                    ...
                    if (srs_resource->transmissionComb.present ==  NR_SRS_Resource__transmissionComb_PR_n2) {
                        ...
                        resource_item->transmissionComb.choice.n2.combOffset_n2  =srs_resource->transmissionComb.choice.n2->combOffset_n2; // choices n2 or n4 uint8_t	 combOffset_n2; // (M) range (0,1) uint8_t	 cyclicShift_n2; // (M) range (0,1,...7)
                        ...
                        resource_item->transmissionComb.choice.n4.combOffset_n4  =srs_resource->transmissionComb.choice.n4->combOffset_n4; // choices n2 or n4 uint8_t	 combOffset_n2; // (M) range (0,1) uint8_t	 cyclicShift_n2; // (M) range (0,1,...7)
                    ...
                    if (srs_resource->resourceType.present == NR_SRS_Resource__resourceType_PR_periodic) {
                        ...
                        resource_item->resourceType.choice.periodic.periodicity =0;// TODO check sturctuce srs_resource->resourceType.choice.periodic->periodicityAndOffset_p.periodicity; //(M) choice periodic (uint8_t periodicity; uint16_t offset);
                        resource_item->resourceType.choice.periodic.offset      =0;// TODO srs_resource->resourceType.choice.periodic->offset; //(M)
                    }
                        ...
                        resource_item->resourceType.choice.aperiodic.aperiodicResourceType = 0; //not done in srs impelmentation srs_resource->resourceType.choice.aperiodic->aperiodicResourceType; //(M)    aperiodic (uint8_t aperiodicResourceType;);
                    }
                        ...
                        resource_item->resourceType.choice.semi_persistent.periodicity =0; // TODO not implemented in SRS srs_resource->resourceType.choice.semi_persistent->periodicity; //(M) semi_persistent (uint8_t periodicity;
                        resource_item->resourceType.choice.semi_persistent.offset      =0; // TODOnot implemented in SRS srs_resource->resourceType.choice.semi_persistent->offset; //(M)
                    }
                ...
                srs_carrier_list_item->active_ul_bwp.sRSConfig.sRSResourceSet_List.srs_resource_set= malloc(maxnoSRSResourceSets*sizeof(f1ap_srs_resource_set_t));
                DevAssert(srs_carrier_list_item->active_ul_bwp.sRSConfig.sRSResourceSet_List.srs_resource_set);
                f1ap_srs_resource_set_t *resourceSet_item=srs_carrier_list_item->active_ul_bwp.sRSConfig.sRSResourceSet_List.srs_resource_set;
                ...
                for(int y=0; y < maxnoSRSResourceSets; y++) { //Preparing SRS Resource Set List
                    ...
                    if(srs_resourceset->resourceType.present== NR_SRS_ResourceSet__resourceType_PR_periodic) {
                        ...
                        resourceSet_item->resourceSetType.choice.periodic.periodicSet=0; // TODO IE not foundsrs_resourceset->resourceType.choice.periodic->associatedCSI_RS;//(M) F1AP_ResourceSetType_t	 resourceSetType;
                    }
                        ...
                        resourceSet_item->resourceSetType.choice.aperiodic.sRSResourceTrigger=0; // TODO verify IE format  srs_resourceset->resourceType.choice.aperiodic->aperiodicSRS_ResourceTriggerList; // TODO IE not found ;//(M) F1AP_ResourceSetType_t	 resourceSetType;
                        resourceSet_item->resourceSetType.choice.aperiodic.slotoffset=0; //TODO verify IE format srs_resourceset->resourceType.choice.aperiodic->slotOffset;
                    }
                        ...
                        resourceSet_item->resourceSetType.choice.semi_persistent.semi_persistentSet=0; // TODO IE not foundsrs_resourceset->resourceType.choice.semi_persistent->associatedCSI_RS;//(M) F1AP_ResourceSetType_t	 resourceSetType;
                    }
                ...
                //Preparing posSRSResource_List IE of SRSConfig (IE of activeULBWP)  TODO IE not found in  OAI srs_config so filled zero values
                ...
                srs_carrier_list_item->active_ul_bwp.sRSConfig.posSRSResource_List.pos_srs_resource_item=malloc(maxnoPosSRSResources*sizeof(f1ap_pos_srs_resource_item_t));
                DevAssert(srs_carrier_list_item->active_ul_bwp.sRSConfig.posSRSResource_List.pos_srs_resource_item);
                f1ap_pos_srs_resource_item_t *pos_resource_item=srs_carrier_list_item->active_ul_bwp.sRSConfig.posSRSResource_List.pos_srs_resource_item;
                LOG_D(MAC,"Preparing posSRSResource_List IE for NRPPA maxnoPosSRSResources=%d \n", srs_carrier_list_item->active_ul_bwp.sRSConfig.posSRSResource_List.pos_srs_resource_list_length);
                for(int z=0; z < maxnoPosSRSResources; z++) { //Preparing Pos SRS Resource List
                    pos_resource_item->srs_PosResourceId=0; // (M)
                    pos_resource_item->transmissionCombPos.n2.combOffset_n2=0; // (M)  f1ap_transmission_comb_pos_n2_t n2 (combOffset_n2,cyclicShift_n2) ; f1ap_transmission_comb_pos_n2_t n4; f1ap_transmission_comb_pos_n8_t n8;
                    ...
                    pos_resource_item->resourceTypePos.periodic.periodicity=0; // (M)    f1ap_resource_type_periodic_pos_t	  periodic;	f1ap_resource_type_semi_persistent_pos_t	semi_persistent;	f1ap_resource_type_aperiodic_pos_t	        aperiodic;
                ...
                //Preparing posSRSResourceSet_List IE of SRSConfig (IE of activeULBWP) TODO IE not found in  OAI srs_config so filled zero values
                int maxnoPosSRSResourceSets = 1;//srs_config->possrs_ResourceSetToAddModList->list.count;
                srs_carrier_list_item->active_ul_bwp.sRSConfig.posSRSResourceSet_List.pos_srs_resource_set_list_length= maxnoPosSRSResourceSets;
                srs_carrier_list_item->active_ul_bwp.sRSConfig.posSRSResourceSet_List.pos_srs_resource_set_item=malloc(maxnoPosSRSResourceSets*sizeof(f1ap_pos_srs_resource_set_item_t));
                DevAssert(srs_carrier_list_item->active_ul_bwp.sRSConfig.posSRSResourceSet_List.pos_srs_resource_set_item);
                f1ap_pos_srs_resource_set_item_t *pos_resourceSet_item=srs_carrier_list_item->active_ul_bwp.sRSConfig.posSRSResourceSet_List.pos_srs_resource_set_item;
                ...
                //  Preparing Uplink Channel BW Per SCS List information IE of SRSCarrier_List f1ap_uplink_channel_bw_per_scs_list_t	 uplink_channel_bw_per_scs_list ; //(M)
                ...
                srs_carrier_list_item->uplink_channel_bw_per_scs_list.scs_specific_carrier = malloc(maxnoSCSs*sizeof(f1ap_scs_specific_carrier_t));
                DevAssert(srs_carrier_list_item->uplink_channel_bw_per_scs_list.scs_specific_carrier);
                f1ap_scs_specific_carrier_t   *scs_specific_carrier_item= srs_carrier_list_item->uplink_channel_bw_per_scs_list.scs_specific_carrier;
