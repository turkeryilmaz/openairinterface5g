    ...
    if (ra->rnti == 0) { // This condition allows for the usage of a preconfigured rnti for the CFRA
      do {
        ...
        loop++;
               ...
               && !((find_nr_UE(&nr_mac->UE_info, CC_id, ra->rnti) == NULL) && (find_nr_RA_id(module_idP, CC_id, ra->rnti) == -1)
                             ...
                             int16_t *tdd_beam_association)
{
  ...
  const int n_slots_frame = nr_slots_per_frame[mu];
  uint8_t k2 = 0;
  if (frame_type == TDD && (RC.ss.mode < SS_SOFTMODEM || RC.ss.mode == SS_SOFTMODEM_SRB)) {
