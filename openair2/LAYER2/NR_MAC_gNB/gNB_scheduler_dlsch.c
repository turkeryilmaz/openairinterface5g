...
void nr_schedule_ue_spec(module_id_t module_id,
  ...
  UE_iterator(UE_info->list, UE) {
    ...
    if (!pdcch_pdu) {
      ...
      for (int i=0;i<6;i++) {
	 if (pdcch_pdu->FreqDomainResource[i]!=0x00)
	     LOG_D(MAC,"CORESET: freq %x\n", (int)pdcch_pdu->FreqDomainResource[i]);
      } 
      LOG_D(NR_MAC,
          "coreset params1: FreqDomainResource %x, start_symbol %d  n_symb %d\n",
          ( unsigned int )pdcch_pdu->FreqDomainResource,
