                  ...
                  uint16_t *rballoc_mask)
{
  ...
  UE_iterator(UE_list, UE) {
    ...
    if (sched_pdsch->dl_harq_pid >= 0) {
      ...
      sched_pdsch->pm_index = mac->identity_pm ? 0 : get_pm_index(mac, UE, sched_pdsch->nrOfLayers, mac->radio_config[CC_id].pdsch_AntennaPorts.XP);
