...
int lc_comparator(const void *p, const void *q)
{
  ...
  nr_lcconfig_t *lc1 = &sched_ctrl1->dl_lc_config[sched_lc1->curr_lcid];
  nr_lcconfig_t *lc2 = &sched_ctrl2->dl_lc_config[sched_lc2->curr_lcid];
...
void fill_lc_list(NR_UE_info_t *UE, frame_t frame, slot_t slot, int *lc_currid, int ue_currid, LCIDsched_t *lc_sched)
{
  ...
  for (uint8_t lc_idx = 0; lc_idx < sched_ctrl->dl_lc_num; lc_idx++) {
    ...
    uint64_t aggregated_pl_lcid = lcid < 4 ? lcid + 1 : sched_ctrl->dl_lc_config[lcid].priority;
    ...
    bool is_gbr_flow = sched_ctrl->dl_lc_config[lcid].guaranteed_bitrate > 0 || lcid < 4;
    
    if (is_gbr_flow) {    /* gbr flow */
      weight_lc = (100 - aggregated_pl_lcid) * (sched_ctrl->dl_lc_config[lcid].Bj > 0 ? 1 : -1);
    }
    else {                                                                     /* non-gbr flow */
...
void nr_update_lc_bucket_parameters(frame_t frame,
  ...
  for (int i = 0; i < sched_ctrl->dl_lc_num; i++) {
    ...
    float T = tti_ms; 
    int64_t bj = lcconfig[lcnum].Bj; 
    bj += lcconfig[lcnum].guaranteed_bitrate * T * 0.001;

    if (lcconfig[lcnum].guaranteed_bitrate == UINT64_MAX)
        bj = INT64_MAX;

    // bj > max bucket size, set bj to max bucket size, as in ts38.321 5.4.3.1 Logical Channel Prioritization
    lcconfig[lcnum].Bj = min(bj, bucketSize_max);

    LOG_D(NR_MAC, "The value of Bj after incrementing is %ld and incremented value is %f\n", lcconfig[lcnum].Bj, lcconfig[lcnum].guaranteed_bitrate * T * 0.001);
  }
}
                              ...
                              float tti_ms)
{
  ...
  uint64_t pbr = sched_ctrl->dl_lc_config[lcnum].guaranteed_bitrate;
 ...
 */
  ...
  if (sched_ctrl->dl_lc_config[lcnum].guaranteed_bitrate > 0) {        // gbr flows
    /*
  ...
  */
    sched_ctrl->dl_lc_config[lcnum].Bj -= num_bytes_reseved_lcid;
    LOG_D(NR_MAC,
          ...
          lcnum,
          sched_ctrl->dl_lc_config[lcnum].Bj);
  }
  else {
  ...
  LOG_D(NR_MAC,
        ...
        sched_ctrl->dl_lc_config[lcnum].guaranteed_bitrate > 0 ? "gbr" : "non-gbr",
        lcnum,
        tbsize_lc,
        num_rbs_reserved_lc,
        oh,
  ...
  */
  ...
  do {
    ...
    LC_iterator(lc_sched, lc)
    {
      ...
      // skip the logical channel if no data in the buffer initially or the data in the buffer was zero because the rbs are already allocated to those bytes
