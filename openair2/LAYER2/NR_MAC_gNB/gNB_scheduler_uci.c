...
static bool get_measured_sinr(int index, int *SINRx10)
{
                                 ...
                                 NR_CSI_ReportConfig__ext2__reportQuantity_r16_PR reportQuantity_type_r16)
{
  ...
  *cumul_bits += 7;
  ...
  bool valid = get_measured_sinr(sinr_index, &sinr_report->SINRx10[0]);
  if (!valid) {
  ...
  sched_ctrl->dl_max_mcs = get_mcs_from_SINRx10(mcs_table, sinr_report->SINRx10[0]);
