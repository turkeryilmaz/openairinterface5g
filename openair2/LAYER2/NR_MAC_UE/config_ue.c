  ...
  if (mcg->schedulingRequestConfig) {
    ...
    if (src->schedulingRequestToAddModList) {
      for (int i = 0; i < src->schedulingRequestToAddModList->list.count; i++) {
        ...
        NR_SR_INFO *sr_info = &si->sr_info[sr->schedulingRequestId];
        sr_info->active_SR_ID = true;
        sr_info->sr_MaxTransmissions = 4 << sr->sr_TransMax;
        int target_ms = 0;
        if (sr->sr_ProhibitTimer)
          target_ms = 1 << *sr->sr_ProhibitTimer;
        // length of slot is (1/2^scs)ms
        nr_timer_setup(&sr_info->sr_ProhibitTimer, target_ms << scs, 1); // 1 slot update rate
      }
    }
  }
  if (mcg->bsr_Config) {
...
static void modify_csi_measconfig(NR_CSI_MeasConfig_t *target, NR_CSI_MeasConfig_t *source)
{
  ...
  switch (csi_MeasConfig_sr->present) {
    ...
    case NR_SetupRelease_CSI_MeasConfig_PR_setup: {
      ...
      if (!sc_info->csi_MeasConfig) { // setup
        ...
        modify_csi_measconfig(sc_info->csi_MeasConfig, csi_MeasConfig_sr->choice.setup);
      }
      break;
    }
    default:
