...
void set_tdd_config_nr_ue(fapi_nr_config_request_t *cfg, int mu, NR_TDD_UL_DL_ConfigCommon_t *tdd_config)
{
  ...
  cfg->tdd_table.max_tdd_periodicity_list =
      (fapi_nr_max_tdd_periodicity_t *)malloc(nb_slots_to_set * sizeof(fapi_nr_max_tdd_periodicity_t));
  ...
  for (int memory_alloc = 0; memory_alloc < nb_slots_to_set; memory_alloc++)
    cfg->tdd_table.max_tdd_periodicity_list[memory_alloc].max_num_of_symbol_per_slot_list =
        (fapi_nr_max_num_of_symbol_per_slot_t *)malloc(NR_NUMBER_OF_SYMBOLS_PER_SLOT
                                                       * sizeof(fapi_nr_max_num_of_symbol_per_slot_t));
  ...
  while (slot_number != nb_slots_to_set) {
    ...
    if (nrofDownlinkSymbols != 0 || nrofUplinkSymbols != 0) {
      ...
      for (int number_of_symbol = nrofDownlinkSymbols; number_of_symbol < NR_NUMBER_OF_SYMBOLS_PER_SLOT - nrofUplinkSymbols;
           number_of_symbol++) {
  ...
  if (tdd_config->pattern1.ext1 == NULL) {
    ...
    AssertFatal(tdd_config->pattern1.ext1->dl_UL_TransmissionPeriodicity_v1530 != NULL,
...
void config_common_ue_sa(NR_UE_MAC_INST_t *mac, module_id_t module_id, int cc_idP)
{
  ...
  cfg->carrier_config.dl_bandwidth =
      get_supported_bw_mhz(*frequencyInfoDL->frequencyBandList.list.array[0]->freqBandIndicatorNR > 256 ? FR2 : FR1, bw_index);
  ...
  for (int i = 0; i < cfg->prach_config.num_prach_fd_occasions; i++) {
    ...
    prach_fd_occasion->k1 =
        ...
        + (get_N_RA_RB(cfg->prach_config.prach_sub_c_spacing,
                       frequencyInfoUL->scs_SpecificCarrierList.list.array[0]->subcarrierSpacing)
           * i);
...
void config_common_ue(NR_UE_MAC_INST_t *mac, module_id_t module_id, int cc_idP)
{
  fapi_nr_config_request_t *cfg = &mac->phy_config.config_req;
  ...
  bw_index = get_supported_band_index(frequencyInfoUL->scs_SpecificCarrierList.list.array[0]->subcarrierSpacing,
                                      *frequencyInfoUL->frequencyBandList->list.array[0],
                                      frequencyInfoUL->scs_SpecificCarrierList.list.array[0]->carrierBandwidth);
  cfg->carrier_config.uplink_bandwidth =
  ...
  for (int i = 0; i < cfg->prach_config.num_prach_fd_occasions; i++) {
    ...
    prach_fd_occasion->prach_zero_corr_conf = rach_ConfigCommon->rach_ConfigGeneric.zeroCorrelationZoneConfig;
    prach_fd_occasion->num_root_sequences = compute_nr_root_seq(rach_ConfigCommon, nb_preambles, frame_type, mac->frequency_range);
    // prach_fd_occasion->num_unused_root_sequences = ???
  }
                          ...
                          const NR_PDCCH_Config_t *pdcch_Config)
{
  ...
  if (pdcch_ConfigCommon) {
    mac->otherSI_SS = pdcch_ConfigCommon->searchSpaceOtherSystemInformation
                          ? get_common_search_space(pdcch_ConfigCommon->commonSearchSpaceList,
                                                    mac,
                                                    *pdcch_ConfigCommon->searchSpaceOtherSystemInformation)
                          : NULL;
    mac->ra_SS = pdcch_ConfigCommon->ra_SearchSpace
                     ? get_common_search_space(pdcch_ConfigCommon->commonSearchSpaceList, mac, *pdcch_ConfigCommon->ra_SearchSpace)
                     : NULL;
    mac->paging_SS =
        pdcch_ConfigCommon->pagingSearchSpace
            ? get_common_search_space(pdcch_ConfigCommon->commonSearchSpaceList, mac, *pdcch_ConfigCommon->pagingSearchSpace)
            : NULL;
  }
  if (pdcch_Config && pdcch_Config->searchSpacesToAddModList) {
    int ss_configured = 0;
    ...
    for (int i = 0; i < searchSpacesToAddModList->list.count; i++) {
      AssertFatal(ss_configured < FAPI_NR_MAX_SS,
                  "Attempting to configure %d SS but only %d per BWP are allowed",
                  ss_configured + 1,
                  FAPI_NR_MAX_SS);
  ...
  if (pdcch_ConfigCommon && pdcch_ConfigCommon->commonControlResourceSet) {
  ...
  if (pdcch_Config && pdcch_Config->controlResourceSetToAddModList) {
    ...
    for (int i = 0; i < controlResourceSetToAddModList->list.count; i++) {
      AssertFatal(cset_configured < FAPI_NR_MAX_CORESET_PER_BWP,
                  "Attempting to configure %d CORESET but only %d per BWP are allowed",
                  cset_configured + 1,
...
void configure_current_BWP(NR_UE_MAC_INST_t *mac, NR_ServingCellConfigCommonSIB_t *scc, NR_CellGroupConfig_t *cell_group_config)
{
  ...
  if (cell_group_config) {
    ...
    if (cell_group_config->spCellConfig && cell_group_config->spCellConfig->spCellConfigDedicated) {
      ...
      UL_BWP->pusch_servingcellconfig =
          spCellConfigDedicated->uplinkConfig && spCellConfigDedicated->uplinkConfig->pusch_ServingCellConfig
              ? spCellConfigDedicated->uplinkConfig->pusch_ServingCellConfig->choice.setup
              : NULL;
      DL_BWP->pdsch_servingcellconfig =
          spCellConfigDedicated->pdsch_ServingCellConfig ? spCellConfigDedicated->pdsch_ServingCellConfig->choice.setup : NULL;
      ...
      if (bwpList && DL_BWP->bwp_id > 0) {
        ...
        configure_ss_coreset(mac,
                             ...
                             spCellConfigDedicated->initialDownlinkBWP->pdcch_Config
                                 ? spCellConfigDedicated->initialDownlinkBWP->pdcch_Config->choice.setup
                                 : NULL);
      }
      ...
      if (ubwpList && UL_BWP->bwp_id > 0) {
        ...
        UL_BWP->configuredGrantConfig = bwp_uplink->bwp_Dedicated->configuredGrantConfig
                                            ? bwp_uplink->bwp_Dedicated->configuredGrantConfig->choice.setup
                                            : NULL;
        ...
        UL_BWP->configuredGrantConfig =
            spCellConfigDedicated->uplinkConfig->initialUplinkBWP->configuredGrantConfig
                ? spCellConfigDedicated->uplinkConfig->initialUplinkBWP->configuredGrantConfig->choice.setup
                : NULL;
  ...
  DL_BWP->initial_BWPSize =
      mac->scc
          ? NRRIV2BW(mac->scc->downlinkConfigCommon->initialDownlinkBWP->genericParameters.locationAndBandwidth, MAX_BWP_SIZE)
  ...
  UL_BWP->initial_BWPSize =
      mac->scc ? NRRIV2BW(mac->scc->uplinkConfigCommon->initialUplinkBWP->genericParameters.locationAndBandwidth, MAX_BWP_SIZE)
  ...
  DL_BWP->initial_BWPStart =
      mac->scc ? NRRIV2PRBOFFSET(mac->scc->downlinkConfigCommon->initialDownlinkBWP->genericParameters.locationAndBandwidth,
                                 MAX_BWP_SIZE)
               : NRRIV2PRBOFFSET(mac->scc_SIB->downlinkConfigCommon.initialDownlinkBWP.genericParameters.locationAndBandwidth,
                                 MAX_BWP_SIZE);
  UL_BWP->initial_BWPStart =
      mac->scc
          ? NRRIV2PRBOFFSET(mac->scc->uplinkConfigCommon->initialUplinkBWP->genericParameters.locationAndBandwidth, MAX_BWP_SIZE)
          : NRRIV2PRBOFFSET(mac->scc_SIB->uplinkConfigCommon->initialUplinkBWP.genericParameters.locationAndBandwidth,
                            MAX_BWP_SIZE);
}
...
void nr_rrc_mac_config_req_mib(module_id_t module_id, int cc_idP, NR_MIB_t *mib, bool sched_sib1)
{
  NR_UE_MAC_INST_t *mac = get_mac_inst(module_id);
...
void nr_rrc_mac_config_req_sib1(module_id_t module_id, int cc_idP, NR_ServingCellConfigCommonSIB_t *scc)
{
  NR_UE_MAC_INST_t *mac = get_mac_inst(module_id);
...
void nr_rrc_mac_config_req_mcg(module_id_t module_id, int cc_idP, NR_CellGroupConfig_t *cell_group_config)
{
...
void nr_rrc_mac_config_req_scg(module_id_t module_id, int cc_idP, NR_CellGroupConfig_t *scell_group_config)
{
  NR_UE_MAC_INST_t *mac = get_mac_inst(module_id);
  RA_config_t *ra = &mac->ra;
