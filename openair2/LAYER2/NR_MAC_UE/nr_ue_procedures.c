...
int8_t nr_ue_decode_paging(NR_UE_MAC_INST_t *mac, nr_downlink_indication_t *dl_info, int pdu_id)
{
  ...
  return nr_mac_rrc_data_ind_ue(module_id, CC_id, gNB_index, frame, slot, mac->physCellId, 0, P_RNTI, PCCH, pduP, pdu_len);
...
void nr_ue_send_sdu(NR_UE_MAC_INST_t *mac, nr_downlink_indication_t *dl_info, int pdu_id)
{
  ...
  switch (dl_info->rx_ind->rx_indication_body[pdu_id].pdu_type){
    ...
    nr_ue_decode_paging(mac, dl_info, pdu_id);
    break;
    default:
    break;
  }
