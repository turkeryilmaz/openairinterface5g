...
int8_t nr_ue_process_dci(module_id_t module_id, int cc_id, uint8_t gNB_index, frame_t frame, int slot, dci_pdu_rel15_t *dci, fapi_nr_dci_indication_pdu_t *dci_ind) {
  ...
  case NR_DL_DCI_FORMAT_1_1: {
    ...
    uint16_t feedback_ti = pucch_Config->dl_DataToUL_ACK->list.array[dci->pdsch_to_harq_feedback_timing_indicator.val][0] + NTN_UE_k2;
...
void nr_get_pusch_tx_power_ue_parameters(NR_UE_MAC_INST_t *mac,
 ...
 if (grant_type == PUSCH_GRANT_TYPE_DCI) {
    ...
    // p0NominalWithGrant
    if(mac->current_UL_BWP.p0_NominalWithGrant)
      P0_NOMINAL_PUSCH = *mac->current_UL_BWP.p0_NominalWithGrant;
    /*
