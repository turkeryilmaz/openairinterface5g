...
static void configure_ratematching_csi(fapi_nr_dl_config_dlsch_pdu_rel15_t *dlsch_pdu,
                                       fapi_nr_dl_config_request_t const *dl_config,
                                       int rnti_type,
                                       int frame,
                                       int slot,
                                       int mu,
                                       int slots_per_frame,
                                       NR_PDSCH_Config_t const *pdsch_config)
{
  ...
  if (pdsch_config && pdsch_config->zp_CSI_RS_ResourceToAddModList) {
    bool found = false;
    NR_SetupRelease_ZP_CSI_RS_ResourceSet_t const *zp_set = pdsch_config->p_ZP_CSI_RS_ResourceSet;
    AssertFatal(zp_set && zp_set->choice.setup, "Only periodic ZP resource set is implemented\n");
    NR_ZP_CSI_RS_Resource_t const *zp_res = NULL;
  ...
  for (int i = 0; i < dl_config->number_pdus; i++) {
    ...
    fapi_nr_dl_config_request_pdu_t const *csi_req = &dl_config->dl_config_list[i];
