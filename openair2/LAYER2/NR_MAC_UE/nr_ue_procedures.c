...
bool get_downlink_ack(NR_UE_MAC_INST_t *mac, frame_t frame, int slot, PUCCH_sched_t *pucch)
{
  ...
  if (!pucch_Config || !pucch_Config->resourceSetToAddModList || pucch_Config->resourceSetToAddModList->list.array[0] == NULL)
...
int8_t nr_ue_get_SR(NR_UE_MAC_INST_t *mac, frame_t frame, slot_t slot, NR_SchedulingRequestId_t sr_id)
{
  ...
  NR_SR_INFO *sr_info = &si->sr_info[sr_id];
  ...
  LOG_D(NR_MAC, "[UE %d] Frame %d slot %d SR %s for ID %ld\n",
        ...
        frame,
        slot,
        sr_info->SR_pending ? "pending" : "not pending",
        sr_id); // todo

  // TODO check if the PUCCH resource for the SR transmission occasion does not overlap with a UL-SCH resource
  if (!sr_info->SR_pending || is_nr_timer_active(sr_info->sr_ProhibitTimer))
    return 0;

  if (sr_info->SR_counter < sr_info->sr_MaxTransmissions) {
    sr_info->SR_counter++;
    // start the sr-prohibittimer
    nr_timer_start(&sr_info->sr_ProhibitTimer);
    LOG_D(NR_MAC, "[UE %d] Frame %d slot %d instruct the physical layer to signal the SR (SR_COUNTER/sr_TransMax %d/%d)\n",
          mac->ue_id,
          frame,
          slot,
          sr_info->SR_counter,
          sr_info->sr_MaxTransmissions);
    return 1;
  }
  ...
  sr_info->SR_pending = false;
  sr_info->SR_counter = 0;
  nr_timer_stop(&sr_info->sr_ProhibitTimer);
                                           ...
                                           CSI_mapping_t mapping_type)
{
      ...
      for (int csi_idx = 0; csi_idx < csi_MeasConfig->nzp_CSI_RS_ResourceSetToAddModList->list.count; csi_idx++) {
        if (csi_MeasConfig->nzp_CSI_RS_ResourceSetToAddModList->list.array[csi_idx]->nzp_CSI_ResourceSetId ==
          ...
          if (mapping_type == ON_PUSCH) {
            ...
            temp_payload_1.word = (0 /*mac->csi_measurements.cri*/ << (cqi_bitlen + ri_bitlen))
                                  | (mac->csirs_measurements.rank_indicator << cqi_bitlen) | (mac->csirs_measurements.cqi);
            temp_payload_2.word = (mac->csirs_measurements.i1 << pmi_x2_bitlen) | mac->csirs_measurements.i2;
          }
          else {
            p1_bits = nr_get_csi_bitlen(csi_report);
            padding_bitlen = p1_bits - (cri_bitlen + ri_bitlen + pmi_x1_bitlen + pmi_x2_bitlen + cqi_bitlen);
            temp_payload_1.word =
                (0 /*mac->csi_measurements.cri*/ << (cqi_bitlen + pmi_x2_bitlen + pmi_x1_bitlen + padding_bitlen + ri_bitlen))
                | (mac->csirs_measurements.rank_indicator << (cqi_bitlen + pmi_x2_bitlen + pmi_x1_bitlen + padding_bitlen))
                | (mac->csirs_measurements.i1 << (cqi_bitlen + pmi_x2_bitlen)) | (mac->csirs_measurements.i2 << (cqi_bitlen))
                | (mac->csirs_measurements.cqi);
          }

          temp_payload_1.byte[0] = reverse_n_bits(temp_payload_1.byte[0], p1_bits);
          temp_payload_2.byte[0] = reverse_n_bits(temp_payload_2.byte[0], p2_bits);
  ...
  csi_payload_t csi;
  csi.part1_payload = temp_payload_1.word;
  csi.part2_payload = temp_payload_2.word;
  csi.p1_bits = p1_bits;
  csi.p2_bits = p2_bits;
  return csi;
}
                                     ...
                                     NR_CSI_MeasConfig_t *csi_MeasConfig)
{
  ...
  csi_payload_t csi;
  csi.part1_payload = temp_payload.w;
  csi.p1_bits = n_bits;
  csi.p2_bits = 0;
  return csi;
}
