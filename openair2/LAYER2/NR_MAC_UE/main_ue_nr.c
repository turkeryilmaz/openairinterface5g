...
void reset_mac_inst(NR_UE_MAC_INST_t *nr_mac)
{
  ...
  for (int i = 0; i < NR_MAX_SR_ID; i++)
    nr_timer_stop(&nr_mac->scheduling_info.sr_info[i].sr_ProhibitTimer);
  ...
  for (int i = 0; i < NR_MAX_SR_ID; i++) {
    nr_mac->scheduling_info.sr_info[i].SR_pending = false;
    nr_mac->scheduling_info.sr_info[i].SR_counter = 0;
  }
