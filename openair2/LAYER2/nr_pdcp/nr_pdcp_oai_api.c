...
{
  DevAssert(deliver_pdu_data == NULL);
  const f1_ue_data_t *ue_data = cu_get_f1_ue_data(ue_id);
  AssertFatal(ue_data != NULL, "nothing found for UE ID %ld\n", ue_id);
...
srb_found:
  if (entity->is_gnb) {
    ...
    const f1_ue_data_t *ue_data = cu_get_f1_ue_data(ue->rntiMaybeUEid);
