  ...
  mem_block_t *const sdu_buffer)
{
  ...
  if (rb != NULL) {
    if (node_type == -1 && get_softmodem_params()->srap) {
      ...
      decode_srap_header(header, (sdu_buffer->data + sdu_buffer_size - header_size));
      if (relay_type == U2N) {
        LOG_I(PDCP, "Rx - bearer id: %x, ue id: %x\n", ((U2NHeader_t*)header)->octet1 & 0x1F, ((U2NHeader_t*)header)->octet2);
      } else if (relay_type == U2U) {
        LOG_I(PDCP, "Rx - bearer id: %x, source ue id: %x, destination ue id: %x\n", ((U2UHeader_t*)header)->octet1 & 0x1F, ((U2UHeader_t*)header)->octet2, ((U2UHeader_t*)header)->octet3);
      }
      rb->recv_pdu(rb, (char *)sdu_buffer->data, sdu_buffer_size - header_size);
    } else {
                            ...
                            char *buf, int size, int sdu_id)
{
  ...
  if (NODE_IS_CU(node_type)) {
    ...
    if (get_softmodem_params()->srap) {
      ...
      U2UHeader_t u2u_header;
      void *header = (relay_type == U2N) ? (void*) &u2n_header: (void*) &u2u_header;
      create_header(relay_type, rb_id, ue_id, 0, header);
      encode_srap_header(header, memblock->data + size);
      if (relay_type == U2N)
        LOG_I(PDCP, "SRAP UE %s(): (drb %d) calling rlc_data_req size %d bearer id %x, ue id: %x\n", __func__, rb_id, size, (((U2NHeader_t*)header)->octet1 & 0x1F), ((U2NHeader_t*)header)->octet2);
      else if (relay_type == U2U)
        LOG_I(PDCP, "SRAP UE %s(): (drb %d) calling rlc_data_req size %d bearer id %x, src ue id: %x dest ue id: %x\n", __func__, rb_id, size, (((U2UHeader_t*)header)->octet1 & 0x1F), ((U2UHeader_t*)header)->octet2, ((U2UHeader_t*)header)->octet3);
