...
int xnap_gNB_handle_handover_request(instance_t instance, sctp_assoc_t assoc_id, uint32_t stream, XNAP_XnAP_PDU_t *pdu)
{
  ...
  if(ie->value.choice.UEContextInfoHORequest.pduSessionResourcesToBeSetup_List.list.count >0) {
    ...
    for (int i=0; i<ie->value.choice.UEContextInfoHORequest.pduSessionResourcesToBeSetup_List.list.count;i++) {
      ...
      OCTET_STRING_TO_INT8(&pdu_session_resources->s_NSSAI.sst, req->ue_context.pdusession_tobe_setup_list.pdu[i].nssai.sst);
   ...
   //BIT_STRING_TO_CHAR_IPv4(&pdu_session_resources->uL_NG_U_TNLatUPF.choice.gtpTunnel->tnl_address, req->ue_context.pdusession_tobe_setup_list.pdu[i].up_ngu_tnl_ip_upf.ipv4_address);
      ...
      for(int j=0; j< pdu_session_resources->qosFlowsToBeSetup_List.list.count;j++) {
	       //req->ue_context.pdusession_tobe_setup_list.pdu[i].qos[j] = ie->value.choice.UEContextInfoHORequest.pduSessionResourcesToBeSetup_List.list.array[i]->qosFlowsToBeSetup_List.list.array[j];
        ...
        req->ue_context.pdusession_tobe_setup_list.pdu[i].qos[j].qfi = qos_flows->qfi;
	req->ue_context.pdusession_tobe_setup_list.pdu[i].qos[j].fiveQI = qos_flows->qosFlowLevelQoSParameters.qos_characteristics.choice.non_dynamic->fiveQI;
	req->ue_context.pdusession_tobe_setup_list.pdu[i].qos[j].allocation_retention_priority.priority_level = qos_flows->qosFlowLevelQoSParameters.qos_characteristics.choice.dynamic->priorityLevelQoS;

      }
    }
  }
  req->ue_context.security_capabilities.encryption_algorithms = BIT_STRING_to_uint16(&ie->value.choice.UEContextInfoHORequest.ueSecurityCapabilities.nr_EncyptionAlgorithms);
  req->ue_context.security_capabilities.integrity_algorithms = BIT_STRING_to_uint16(&ie->value.choice.UEContextInfoHORequest.ueSecurityCapabilities.nr_IntegrityProtectionAlgorithms);
