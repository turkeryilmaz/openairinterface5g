...
*/
  ...
  if (xnap_gNB_encode_pdu(&pdu, &buffer, &len) < 0) {
    XNAP_ERROR("Failed to encode Xn setup request\n");
    return -1;
  }
  printf(" going to send xn setup request");
                                       ...
                                       long time_to_wait)
{
  ...
  LOG_I(XNAP, "Encoding PDU successfull");
  LOG_I(XNAP, "Sending from 0xe01");
