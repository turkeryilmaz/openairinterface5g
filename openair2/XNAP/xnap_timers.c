    ...
    int t_dc_overall)
{
  ...
  t->txn_reloc_overall = txn_reloc_overall;
...
void xnap_check_timers(instance_t instance)
{
  ...
  for (i = 0; i < XNAP_MAX_IDS; i++) {
    ...
    if (m->ids[i].state == XNID_STATE_SOURCE_PREPARE ||
        m->ids[i].state == XNID_STATE_SOURCE_OVERALL)
      xn_ongoing++;

    if (m->ids[i].state == XNID_STATE_SOURCE_PREPARE &&
        t->tti > m->ids[i].t_reloc_prep_start + t->t_reloc_prep) {
      LOG_I(XNAP, "XN timeout reloc prep\n");
      /* t_reloc_prep timed out */
      cause = XNAP_T_RELOC_PREP_TIMEOUT;
      goto nr_handover_timeout;
    }

    if (m->ids[i].state == XNID_STATE_SOURCE_OVERALL &&
        t->tti > m->ids[i].txn_reloc_overall_start + t->txn_reloc_overall) {
      LOG_I(XNAP, "XN timeout reloc overall\n");
      /* txn_reloc_overall timed out */
      cause = XNAP_TXN_RELOC_OVERALL_TIMEOUT;
      goto nr_handover_timeout;
    }

    if (m->ids[i].state == XNID_STATE_NSA_GNB_PREPARE &&
        t->tti > m->ids[i].t_dc_prep_start + t->t_dc_prep) {
      int id_source;
      int id_target;

      LOG_I(XNAP, "XN timeout DC prep\n");
      /* t_dc_prep timed out */
      target = xnap_id_get_target(m, i);
      id_source = xnap_id_get_id_source(m, i);
      id_target = xnap_id_get_id_target(m, i);
      xnap_gNB_generate_ENDC_xn_SgNB_release_request(instance_p, target,
                                                     id_source, id_target,
    ...
    if (m->ids[i].state == XNID_STATE_NSA_GNB_OVERALL &&
        t->tti > m->ids[i].t_dc_overall_start + t->t_dc_overall) {
      int id_source;
      int id_target;

      LOG_I(XNAP, "XN timeout DC overall\n");
      /* t_dc_overall timed out */
      target = xnap_id_get_target(m, i);
      id_source = xnap_id_get_id_source(m, i);
      id_target = xnap_id_get_id_target(m, i);
      xnap_gNB_generate_ENDC_xn_SgNB_release_required(instance_p, target,
...
nr_handover_timeout:
    ...
    target = xnap_id_get_target(m, i);
    xnap_gNB_generate_xn_handover_cancel(instance_p, target, i, cause);
  ...
  if (xn_ongoing && t->tti % 1000 == 0)
    LOG_I(XNAP, "XN has %d process ongoing\n", xn_ongoing);
}
#endif
