...
static int nr_ulsch_procedures(PHY_VARS_gNB *gNB, int frame_rx, int slot_rx, bool *ulsch_to_decode, NR_UL_IND_t *UL_INFO)
{
  ...
  int ret_nr_ulsch_decoding = nr_ulsch_decoding(gNB, frame_parms, frame_rx, slot_rx, G, ULSCH_ids, nb_pusch, UL_INFO);
  ...
  for (pusch_id = 0; pusch_id < nb_pusch; pusch_id++) {
    ...
    if (crc_valid && !check_abort(&ulsch_harq->abort_decode) && !pusch->DTX) {
      ...
      nr_fill_indication(gNB, ulsch->frame, ulsch->slot, ULSCH_id, ulsch->harq_pid, 0, 0, UL_INFO);
      ...
      LOG_D(PHY,
            ...
            ulsch_harq->TBS);
      nr_fill_indication(gNB, ulsch->frame, ulsch->slot, ULSCH_id, ulsch->harq_pid, 1, 0, UL_INFO);
      ulsch->handled = 1;
                        ...
                        NR_UL_IND_t *UL_INFO)
{
  ...
  // crc indication
  UL_INFO->crc_ind.sfn = frame;
  UL_INFO->crc_ind.slot = slot_rx;
  // absurd data model, here we use local storage, in NFAPI, we use malloc
    ...
    if ((ulsch->active == true) && (ulsch->frame == frame_rx) && (ulsch->slot == slot_rx) && (ulsch->handled == 0)) {
      ...
      if (dB_fixed_x10(pusch_vars->ulsch_power_tot) < dB_fixed_x10(pusch_vars->ulsch_noise_power_tot) + gNB->pusch_thres) {
        ...
        if (!get_softmodem_params()->phy_test) {
          ...
          nr_fill_indication(gNB, frame_rx, slot_rx, ULSCH_id, ulsch->harq_pid, 1, 1, UL_INFO);
          pusch_DTX++;
          continue;
        }
      } else {
