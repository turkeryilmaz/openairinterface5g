  ...
  for (int i = 0; i < UL_tti_req->n_pdus; i++) {
    switch (UL_tti_req->pdus_list[i].pdu_type) {
      ...
      case NFAPI_NR_UL_CONFIG_PRACH_PDU_TYPE:
        ...
        gNB_PRACH_list_t *prach = nr_fill_prach(gNB, UL_tti_req->SFN, UL_tti_req->Slot, prach_pdu);
        if (prach && (gNB->RU_list[0]->if_south == LOCAL_RF || gNB->RU_list[0]->if_south == REMOTE_IF5))
          nr_fill_prach_ru(gNB->RU_list[0], UL_tti_req->SFN, UL_tti_req->Slot, prach_pdu, prach->beam_nb);
        break;
