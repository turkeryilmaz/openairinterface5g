...
void L1_nr_prach_procedures(PHY_VARS_gNB *gNB, int frame, int slot, NR_UL_IND_t *UL_INFO)
{
  ...
  UL_INFO->rach_ind.sfn = frame;
  UL_INFO->rach_ind.slot = slot;
  ...
  if (prach_id>=0) {
	...
	gNB->prach_vars.rxsigF[aa] = ru->prach_rxsigF[prach_oc][ru_aa];
      ...
      if ((gNB->prach_energy_counter == 100) && (max_preamble_energy[0] > gNB->measurements.prach_I0 + gNB->prach_thres)
          && (UL_INFO->rach_ind.number_of_pdus < MAX_NUM_NR_RX_RACH_PDUS)) {
        LOG_A(NR_PHY,
        ...
        nfapi_nr_prach_indication_pdu_t *ind = UL_INFO->prach_pdu_indication_list + UL_INFO->rach_ind.number_of_pdus;
