  ...
  while (pdu->pdu_type != FAPI_NR_END) {
    switch (pdu->pdu_type) {
      case FAPI_NR_UL_CONFIG_TYPE_PUSCH: {
        ...
        phy_data->ulsch.pusch_pdu = pdu->pusch_config_pdu;
        if (tx_request) {
          ...
          for (int j = 0; j < tx_request->number_of_pdus; j++) {
            ...
            if (tx_req_body->pdu_index == pdu && tx_req_body->pdu_length > 0) {
              LOG_D(PHY, "Copying %d bytes to harq_process_ul_ue->a (harq_pid %d)\n", tx_req_body->pdu_length, current_harq_pid);
      ...
      case FAPI_NR_UL_CONFIG_TYPE_SRS:
        // srs config pdu
        // Weird: we copy to "phy" instead of "phy_data" ???
...
{
  PHY_VARS_NR_UE *phy = PHY_vars_UE_g[scheduled_response->module][scheduled_response->carrier];
