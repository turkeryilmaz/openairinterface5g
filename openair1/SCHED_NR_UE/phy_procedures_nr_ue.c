                            ...
                            c16_t rxdataF[ue->frame_parms.nb_antennas_rx][ALNARS_32_8(ue->frame_parms.ofdm_symbol_size)])
{
  ...
  const nr_phy_data_t *phy_data = (nr_phy_data_t*)dl_indication->phy_data;
  ...
  if (symbol == slot_data->stopSymbPdcch) {
    nr_pdcch_dci_indication(proc, slot_data->pdcchLlrSize, slot_data->numMonitoringOcc, ue, &phy_data->phy_pdcch_config, dl_indication, slot_data->pdcchLlr);
