...
/*! \file phy_procedures_nr_ue_sl.c
 * \brief UE phy scheduler functions to decode/encode sidelink physical channels
 * \author
 * \date
 * \version 0.1
 * \company Fraunhofer
  ...
  switch (pdu_type){
    ...
    case FAPI_NR_RX_PDU_TYPE_SSB: {
        ...
        if(typeSpecific) {
          ...
          ssb_pdu->rsrp_dbm = sl_phy_params->psbch_rx.rsrp_dBm_per_RE;
          ssb_pdu->rx_slss_id = sl_phy_params->sl_config.sl_sync_source.rx_slss_id;
...
static int nr_ue_psbch_procedures(PHY_VARS_NR_UE *ue,
  ...
  uint8_t *result = NULL;
  if (ret) sl_phy_params->psbch_rx.rx_errors ++;
  else {
    result = decoded_pdu;
    sl_phy_params->psbch_rx.rx_ok ++;
  }

  nr_fill_sl_indication(&sl_indication, &rx_ind, NULL, proc, ue, phy_data);
  nr_fill_sl_rx_indication(&rx_ind, SL_NR_RX_PDU_TYPE_SSB, ue, number_pdus, proc, (void *)result);
...
//    nr_ue_rrc_measurements(ue, proc, rxdataF);
    ...
    if (frame_rx%64 == 0) {
      ...
      LOG_I(NR_PHY,"[UE%d] PSBCH Stats: TX %d, RX ok %d, RX not ok %d\n",
                              ue->Mod_id, sl_phy_params->psbch_tx.num_psbch_tx,
                              sl_phy_params->psbch_rx.rx_ok,
                              sl_phy_params->psbch_rx.rx_errors);
...
#if 0
  int tx_offset = sl_phy_params->sl_frame_params.get_samples_slot_timestamp(slot_tx, &sl_phy_params->sl_frame_params, 0);

  for (int i=0;i<2048;i++) {
    printf("%d, %d:%d\n", tx_offset+i, ue->common_vars.txdata[0][tx_offset + i].r, ue->common_vars.txdata[0][tx_offset + i].i);
  }
#endif
