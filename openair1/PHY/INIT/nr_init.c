...
void phy_free_nr_gNB(PHY_VARS_gNB *gNB)
{
  ...
  for (int ULSCH_id = 0; ULSCH_id < gNB->max_nb_pusch; ULSCH_id++) {
    ...
    for (int i=0; i< max_ul_mimo_layers; i++)
      free_and_zero(pusch->llr_layers[i]);
    for (int i = 0; i < Prx; i++) {
      free_and_zero(pusch->rxdataF_ext[i]);
      for (int j=0; j< max_ul_mimo_layers; j++) {
        for (int k=0; k<max_ul_mimo_layers; k++)
          free_and_zero(pusch->rho[i][j * max_ul_mimo_layers + k]);
      }
      free_and_zero(pusch->rho[i]);
    }
    for (int i = 0; i < n_buf; i++) {
      free_and_zero(pusch->ul_ch_estimates[i]);
      free_and_zero(pusch->ul_ch_estimates_ext[i]);
      free_and_zero(pusch->ul_ch_estimates_time[i]);
      free_and_zero(pusch->ptrs_phase_per_slot[i]);
      free_and_zero(pusch->rxdataF_comp[i]);
      free_and_zero(pusch->ul_ch_mag0[i]);
      free_and_zero(pusch->ul_ch_magb0[i]);
      free_and_zero(pusch->ul_ch_magc0[i]);
      free_and_zero(pusch->ul_ch_mag[i]);
      free_and_zero(pusch->ul_ch_magb[i]);
      free_and_zero(pusch->ul_ch_magc[i]);
    }
    free_and_zero(pusch->llr_layers);
    free_and_zero(pusch->rxdataF_ext);
    free_and_zero(pusch->ul_ch_estimates);
    free_and_zero(pusch->ul_ch_estimates_ext);
    free_and_zero(pusch->ptrs_phase_per_slot);
    free_and_zero(pusch->ul_ch_estimates_time);
    free_and_zero(pusch->ul_valid_re_per_slot);
    free_and_zero(pusch->rxdataF_comp);
    free_and_zero(pusch->ul_ch_mag0);
    free_and_zero(pusch->ul_ch_magb0);
    free_and_zero(pusch->ul_ch_magc0);
    free_and_zero(pusch->ul_ch_mag);
    free_and_zero(pusch->ul_ch_magb);
    free_and_zero(pusch->ul_ch_magc);
    free_and_zero(pusch->rho);

    free_and_zero(pusch->llr);
  } // ULSCH_id
  free(gNB->pusch_vars);
}
