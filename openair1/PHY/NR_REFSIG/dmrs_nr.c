...
void nr_chest_time_domain_avg(const NR_DL_FRAME_PARMS *frame_parms,
                              ...
                              const int layerIdx,
                              const int Nl,
                              const bool isUE, // To handle different ch_estimates buffer format for UE
                              c16_t *ch_estimates)
{
  ...
  const int num_dmrs_symb = get_dmrs_symbols_in_slot(dmrs_bitmap, total_symbols, start_symbol);
