			 ...
			 int madd)
{
  ...
  for(i=0; i<(N>>2); i++) {
    ...
    tmp_im = oai_mm_sign_epi16(tmp_im,*(simde__m128i*)&conjug[0]);
                    ...
                    int output_shift)
{
 ...
 // print_shorts("x2_128:",&x2_128[0]);
  ...
  // we compute 4 cpx multiply for each loop
  for(i=0; i<(N>>2); i++) {
    tmp_re = oai_mm_sign_epi16(*x1_128,*(simde__m128i*)&conjug2[0]);// Q15
                    ...
                    int output_shift)
{
  ...
  // we compute 4 cpx multiply for each loop
  for(i=0; i<(N>>2); i++) {
    tmp_re = oai_mm_sign_epi16(*x1_128,*(simde__m128i*)&conjug2[0]);
