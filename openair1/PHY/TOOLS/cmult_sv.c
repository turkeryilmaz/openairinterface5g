...
void mult_real_vector_single_vector(const c16_t *x, const c16_t *alpha, c16_t *y, const unsigned int N)
{
  const simd_q15_t *alpha_128 = (const simd_q15_t *)alpha;
  const simd_q15_t *x_128 = (const simd_q15_t *)x;
  simd_q15_t *y_128 = (simd_q15_t *)y;
  const unsigned int num_adds = (N + 4 - 1) / 4;
                                        ...
                                        uint32_t N)
{
  ...
  const uint32_t num_adds = (N + 4 - 1) / 4;
