                       ...
                       uint16_t output_shift)
{
  ...
  if ( (intptr_t)x%32 == 0  && !(intptr_t)y%32 == 0 && __builtin_cpu_supports("avx2")) {
    ...
    simde__m256i const alpha_for_real =  simde_mm256_set1_epi32(*(uint32_t*)&for_re);
    const c16_t for_im={alpha->i, alpha->r};
    simde__m256i const alpha_for_im= simde_mm256_set1_epi32(*(uint32_t*)&for_im);
    simde__m256i const perm_mask =
    ...
    for( simde__m256i* yd = (simde__m256i *)y; xd<end ; yd++, xd++) {
      const simde__m256i xre = simde_mm256_srai_epi32(simde_mm256_madd_epi16(*xd,alpha_for_real),
						 output_shift);
      const simde__m256i xim = simde_mm256_srai_epi32(simde_mm256_madd_epi16(*xd,alpha_for_im),
						 output_shift);
