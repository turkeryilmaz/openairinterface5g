...
static
__attribute__((always_inline)) inline c16_t c32x16cumulVectVectWithSteps(c16_t const* in1,
                                                                         int offset1,
                                                                         const int step1,
                                                                         c16_t const* in2,
                                                                         int offset2,
  ...
  c32_t cumul={0};
  for (int i=0; i<N; i++) {
    cumul=c32x16maddShift(in1[offset1 + i*step1], in2[(offset2 + i*step2)%modulo2], cumul, 15);
//    localOffset1+=step1;
...
int nr_pusch_channel_estimation(PHY_VARS_gNB *gNB,
  ...
  const int k = bwp_start_subcarrier;
  ...
  if (tmp_k > symbolSize) {
	...
	l0 = (symbolSize - k)/12;
	have_half_prb = (symbolSize-k)%12;
	l1 = (tmp_k - symbolSize)/12;
  }
  else l0 = nb_rb_pusch;  
    ...
    if (pusch_pdu->dmrs_config_type == pusch_dmrs_type1 && chest_freq == 0) {
      ...
      for (int n = 0; n < 3 * nb_rb_pusch; n++) {
        ...
        for (int k_line = 0; k_line <= 1; k_line++) {
          re_offset = (k + (n << 2) + (k_line << 1) + delta) % symbolSize;
...
#ifdef DEBUG_PUSCH
          re_offset = (k + (n << 2) + (k_line << 1)) % symbolSize;
      ...
      for (int n = 0; n < nb_rb_pusch * NR_NB_SC_PER_RB; n += 6) {
        c16_t ch0 = c16mulShift(*pil, rx[(k + n) % symbolSize], 15);
        pil++;
        c16_t ch1 = c16mulShift(*pil, rx[(k + n + 1) % symbolSize], 15);
        pil++;
