...
__device__ __forceinline__ int32_t __vxor4(const uint32_t *b, uint32_t *a)
{
    return *b ^ *a; //| 0x01010101;
}

__device__ __forceinline__ int32_t __vsign4(const uint32_t *b, uint32_t *a)
{
                                     ...
                                     int Zc)
{
    ...
    const uint32_t *d_in = d_cnBufAll + blockIdx.x * NUM * Zc; // input pointer each block tackle with
    uint32_t *d_out = d_cnOutAll + blockIdx.x * NUM * Zc;      // output pointer each block tackle with
                                      ...
                                      int Zc)
{
    ...
    const uint32_t *d_in = d_cnBufAll + blockIdx.x / 2 * NUM * Zc; // input pointer each block tackle with
    uint32_t *d_out = d_cnOutAll + blockIdx.x / 2 * NUM * Zc;      // output pointer each block tackle with
                       ...
                       uint16_t Z)
{
    ...
    const uint32_t *lut_numCnInCnGroups = p_lut->numCnInCnGroups;
