...
__device__ void bnProcPcKernel_int8_Gn(
...
__device__ void bnProcKernel_int8_Gn(
    const int8_t *__restrict__ d_bnProcBuf,
    const int8_t *__restrict__ d_bnProcBufRes,
    const int8_t *__restrict__ d_llrProcBuf,
    const int8_t *__restrict__ d_llrRes,
...
{
    printf(
        "bnProcKernel_int8_Gn Debug | lane=%d | GrpIdx=%d | MsgIdx=%d | BnIdx=%d | GrpNum=%d | Zc=%d | ymm0Res=0x%08x | prevMsg=0x%08x | MsgRes=0x%08x\n",
