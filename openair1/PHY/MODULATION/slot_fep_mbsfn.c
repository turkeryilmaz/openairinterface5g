  ...
  for (aa=0; aa<frame_parms->nb_antennas_rx; aa++) {
    ...
    if (l==0) {
      ...
      dft(dftsizeidx,(int16_t *)&common_vars->rxdata[aa][(sample_offset +
          ...
          (int16_t *)&common_vars->common_vars_rx_data_per_thread[ue->current_thread_id[subframe]].rxdataF[aa][frame_parms->ofdm_symbol_size*l],get_dft_scaling(s));
...
int slot_fep_mbsfn_khz_1dot25(PHY_VARS_UE *ue,
  ...
  for (aa=0; aa<frame_parms->nb_antennas_rx; aa++) {
    ...
    dft(get_dft(ofdm_symbol_size),(int16_t *)&common_vars->rxdata[aa][(sample_offset +
                                            ...
                                            subframe_offset -
                                            SOFFSET) % frame_length_samples],
        (int16_t *)&common_vars->common_vars_rx_data_per_thread[ue->current_thread_id[subframe]].rxdataF[aa][0],get_dft_scaling(ofdm_symbol_size));
    stop_UE_TIMING(ue->rx_dft_stats);
  }
