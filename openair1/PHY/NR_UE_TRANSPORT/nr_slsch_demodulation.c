...
void nr_slsch_extract_rbs(int32_t **rxdataF,
  ...
  for (aarx = 0; aarx < frame_parms->nb_antennas_rx; aarx++) {
    //uint16_t m0 = 0;
    uint16_t m = 0;
    uint32_t tmpk = soffset + (symbol * frame_parms->ofdm_symbol_size);
    ...
    if (is_dmrs_symbol == 0) {
      ...
      for (aatx = 0; aatx < pssch_pdu->nrOfLayers; aatx++) {
        ...
        sl_ch0_ext_index = 0;
        sl_ch0_index = 0;
        for (re = 0; re < nb_re_pssch; re++, tmpk++) {
...
void nr_slsch_scale_channel(int **sl_ch_estimates_ext,
                            NR_DL_FRAME_PARMS *frame_parms,
                            NR_UE_DLSCH_t *dlsch,
                            uint8_t symbol,
                            uint8_t is_dmrs_symbol,
                            ...
                            unsigned short nb_rb)
{
  ...
  ch_amp = 1024*8; //((pilots) ? (ulsch_nrUE->sqrt_rho_b) : (ulsch_nrUE->sqrt_rho_a));
  ...
  for (aatx = 0; aatx < nrOfLayers; aatx++) {
    for (aarx=0; aarx < frame_parms->nb_antennas_rx; aarx++) {
      ul_ch128 = (__m128i *)&sl_ch_estimates_ext[aatx*frame_parms->nb_antennas_rx+aarx][symbol*(off+(nb_rb*NR_NB_SC_PER_RB))];
                            ...
                            unsigned short nb_rb)
{
  ...
  for (aatx = 0; aatx < nrOfLayers; aatx++) {
    for (aarx = 0; aarx < frame_parms->nb_antennas_rx; aarx++) {
      ...
      ul_ch128=(__m128i *)&sl_ch_estimates_ext[aatx*frame_parms->nb_antennas_rx+aarx][symbol*(off+(nb_rb*12))];
  ...
  for (aatx=0; aatx<nrOfLayers; aatx++) {
    for (aarx=0; aarx<frame_parms->nb_antennas_rx; aarx++) {
      ...
      ul_ch128 = (int16x4_t *)&sl_ch_estimates_ext[aatx*frame_parms->nb_antennas_rx+aarx][symbol*frame_parms->N_RB_UL*12];
...
void nr_slsch_channel_compensation(int **rxdataF_ext,
  ...
  for (int nl=0; nl<nrOfLayers; nl++) {
    for (int aarx = 0; aarx < frame_parms->nb_antennas_rx; aarx++) {
      ...
      ul_ch = (int16_t *) &sl_ch_estimates_ext[nl * frame_parms->nb_antennas_rx + aarx][symbol * (off + (nb_rb * 12))];
    ...
    for (aarx=0; aarx < frame_parms->nb_antennas_rx; aarx++) {
      for (aatx=0; aatx < nrOfLayers; aatx++) {
        for (int atx=0; atx< nrOfLayers; atx++) {
          ...
          ul_ch128      = (__m128i *)&sl_ch_estimates_ext[aatx*frame_parms->nb_antennas_rx+aarx][symbol*(off+(nb_rb*12))];
          ul_ch128_2    = (__m128i *)&sl_ch_estimates_ext[atx*frame_parms->nb_antennas_rx+aarx][symbol*(off+(nb_rb*12))];
  ...
  printf("This gets rid of unused variable warning! rxdataF_comp128 %p, rxdataF128 %p\n", rxdataF_comp128, rxdataF128);
  ...
  if (rho) {
    for (aarx=0; aarx<frame_parms->nb_antennas_rx; aarx++) {
      ...
      ul_ch128      = (int16x4_t*)&sl_ch_estimates_ext[aarx][symbol*frame_parms->N_RB_UL*12];
      ul_ch128_2    = (int16x4_t*)&sl_ch_estimates_ext[2+aarx][symbol*frame_parms->N_RB_UL*12];
      for (rb=0; rb<nb_rb; rb++) {
...
void nr_slsch_detection_mrc(NR_DL_FRAME_PARMS *frame_parms,
...
/* Zero Forcing Rx function: nr_slsch_zero_forcing_rx_2layers()
 *
 *
 * */
uint8_t nr_slsch_zero_forcing_rx_2layers(int **rxdataF_comp,
                                   int **ul_ch_mag,
                                   int **ul_ch_magb,
                                   int **sl_ch_estimates_ext,
                                   unsigned short nb_rb,
                                   ...
                                   int length)
{
  ...
  switch (n_rx) {
    case 2://
      ch00 = (int *)&sl_ch_estimates_ext[0][symbol*(off+nb_rb*12)];
      ch01 = (int *)&sl_ch_estimates_ext[2][symbol*(off+nb_rb*12)];
      ch10 = (int *)&sl_ch_estimates_ext[1][symbol*(off+nb_rb*12)];
      ch11 = (int *)&sl_ch_estimates_ext[3][symbol*(off+nb_rb*12)];
      ch20 = NULL;
      ch21 = NULL;
      ch30 = NULL;
      ch31 = NULL;
    ...
    case 4://
      ch00 = (int *)&sl_ch_estimates_ext[0][symbol*(off+nb_rb*12)];
      ch01 = (int *)&sl_ch_estimates_ext[4][symbol*(off+nb_rb*12)];
      ch10 = (int *)&sl_ch_estimates_ext[1][symbol*(off+nb_rb*12)];
      ch11 = (int *)&sl_ch_estimates_ext[5][symbol*(off+nb_rb*12)];
      ch20 = (int *)&sl_ch_estimates_ext[2][symbol*(off+nb_rb*12)];
      ch21 = (int *)&sl_ch_estimates_ext[6][symbol*(off+nb_rb*12)];
      ch30 = (int *)&sl_ch_estimates_ext[3][symbol*(off+nb_rb*12)];
      ch31 = (int *)&sl_ch_estimates_ext[7][symbol*(off+nb_rb*12)];
      break;
                 ...
                 unsigned char harq_pid)
{
  ...
  for(uint8_t symbol = rel16_sl->start_symbol_index; symbol < (rel16_sl->start_symbol_index + rel16_sl->nr_of_symbols); symbol++) {
    ...
    LOG_D(NR_PHY, "symbol %d, dmrs_symbol_flag :%d\n", symbol, dmrs_symbol_flag);
    ...
    if (dmrs_symbol_flag == 1) {
      ...
      for (int nl=0; nl<rel16_sl->nrOfLayers; nl++) {
        nr_pssch_channel_estimation(nrUE,
                                    slot,
...
#if 0
      nr_ue_measurements(nrUE, proc, slot);
      //nr_gnb_measurements(nrUE, dlsch_id, harq_pid, symbol,rel16_sl->nrOfLayers);

      for (aarx = 0; aarx < frame_parms->nb_antennas_rx; aarx++) {
        if (symbol == rel16_sl->start_symbol_index) {
          nrUE->pssch_vars[dlsch_id]->slsch_power[aarx] = 0;
          nrUE->pssch_vars[dlsch_id]->slsch_noise_power[aarx] = 0;
        }
        for (aatx = 0; aatx < rel16_sl->nrOfLayers; aatx++) {
          nrUE->pssch_vars[dlsch_id]->slsch_power[aarx] += signal_energy_nodc(
            &nrUE->pssch_vars[dlsch_id]->sl_ch_estimates[aatx*nrUE->frame_parms.nb_antennas_rx+aarx][symbol * frame_parms->ofdm_symbol_size],
            rel16_sl->rb_size * 12);
        }

        for (int rb = 0; rb < rel16_sl->rb_size; rb++) {
          nrUE->pssch_vars[dlsch_id]->slsch_noise_power[aarx] +=
              nrUE->measurements.n0_subband_power[aarx][rel16_sl->bwp_start + rel16_sl->rb_start + rb] /
              rel16_sl->rb_size;
        }
        LOG_D(NR_PHY, "aa %d, bwp_start%d, rb_start %d, rb_size %d: slsch_power %d, slsch_noise_power %d\n",aarx,
	      rel16_sl->bwp_start,rel16_sl->rb_start,rel16_sl->rb_size,
              nrUE->pssch_vars[dlsch_id]->slsch_power[aarx],
              nrUE->pssch_vars[dlsch_id]->slsch_noise_power[aarx]);
      }
#endif
    }
  ...
  if (nrUE->chest_time == 1) { // averaging time domain channel estimates
    nr_chest_time_domain_avg(frame_parms,
                             ...
                             rel16_sl->ul_dmrs_symb_pos,
                             rel16_sl->rb_size);

    nrUE->pssch_vars[dlsch_id]->dmrs_symbol = get_next_dmrs_symbol_in_slot(rel16_sl->ul_dmrs_symb_pos, rel16_sl->start_symbol_index, rel16_sl->nr_of_symbols);
  }
  ...
  for(uint8_t symbol = rel16_sl->start_symbol_index; symbol < (rel16_sl->start_symbol_index + rel16_sl->nr_of_symbols); symbol++) {
    ...
    if (nb_re_pusch > 0) {
      ...
      //--------------------- Channel Scaling --------------------
      //----------------------------------------------------------
      nr_slsch_scale_channel(nrUE->pssch_vars[dlsch_id]->sl_ch_estimates_ext,
                            frame_parms,
                            slsch,
                            symbol,
                            dmrs_symbol_flag,
      ...
      if (nrUE->pssch_vars[dlsch_id]->cl_done==0) {
        nr_slsch_channel_level(nrUE->pssch_vars[dlsch_id]->sl_ch_estimates_ext,
                              frame_parms,
                              avg,
                              symbol,
                              nb_re_pusch,
      ...
      //----------------------------------------------------------
      start_meas(&nrUE->slsch_channel_compensation_stats);
      LOG_D(NR_PHY, "Doing channel compensations log2_maxh %d, avgs %d (%d,%d)\n",nrUE->pssch_vars[dlsch_id]->log2_maxh,avgs,avg[0],avg[1]);
      nr_slsch_channel_compensation(nrUE->pssch_vars[dlsch_id]->rxdataF_ext,
                                    nrUE->pssch_vars[dlsch_id]->sl_ch_estimates_ext,
                                    nrUE->pssch_vars[dlsch_id]->sl_ch_mag0,
                                    nrUE->pssch_vars[dlsch_id]->sl_ch_magb0,
      ...
      stop_meas(&nrUE->slsch_channel_compensation_stats);

      start_meas(&nrUE->slsch_mrc_stats);
      nr_slsch_detection_mrc(frame_parms,
                             nrUE->pssch_vars[dlsch_id]->rxdataF_comp,
                             nrUE->pssch_vars[dlsch_id]->sl_ch_mag0,
                             nrUE->pssch_vars[dlsch_id]->sl_ch_magb0,
                             (rel16_sl->nrOfLayers>1) ? nrUE->pssch_vars[dlsch_id]->rho : NULL,
      ...
      if (rel16_sl->nrOfLayers == 2)//Apply zero forcing for 2 Tx layers
        nr_slsch_zero_forcing_rx_2layers(nrUE->pssch_vars[dlsch_id]->rxdataF_comp,
                                   nrUE->pssch_vars[dlsch_id]->sl_ch_mag0,
                                   nrUE->pssch_vars[dlsch_id]->sl_ch_magb0,
                                   nrUE->pssch_vars[dlsch_id]->sl_ch_estimates_ext,
                                   rel16_sl->rb_size,
                                   ...
                                   symbol,
                                   nb_re_pusch);
      stop_meas(&nrUE->slsch_mrc_stats);
