                           ...
                           uint8_t *DLSCH_ids)
{
  ...
  for (uint8_t pdsch_id = 0; pdsch_id < nb_dlsch; pdsch_id++) {
     ...
     */
    TB_decoding_params->harq_unique_pid = 2 * harq_pid + DLSCH_id;
  ...
  int number_tasks_decode = phy_vars_ue->nrLDPC_coding_interface.nrLDPC_coding_decoder(&slot_decoding_params);
