...
static void nr_scan_ssb(void *arg)
{
  ...
  DevAssert(ssbInfo->rxdata_sz > 0 && fp->ofdm_offset_divisor > 0);
  const uint32_t capture_samples = ssbInfo->rxdata_sz;
                                  ...
                                  int numGscn)
{
  ...
  DevAssert(input_sz > 0);
  const int capture_samples = input_sz;
  ...
  for (int ant = 0; ant < fp->nb_antennas_rx; ant++) {
    ...
    copy_freq_compensated(input[ant], normalized_rxdata[ant], capture_samples, ue->initial_fo, fp->samples_per_subframe * 1000, 0);
  }
