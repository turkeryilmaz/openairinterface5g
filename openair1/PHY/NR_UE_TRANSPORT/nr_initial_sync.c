...
int nr_sl_initial_sync(UE_nr_rxtx_proc_t *proc, PHY_VARS_NR_UE *ue, int n_frames)
{
  ...
  for (int is = 0; is < n_frames; is++) {
    sync_pos = pss_synchro_nr(ue, is, NO_RATE_CHANGE); // NOT the location where the slot boundry is set
    ...
    if (sync_pos < (NR_NUMBER_OF_SUBFRAMES_PER_FRAME * fp->samples_per_subframe - (N_SYMB_SLOT_SL * fp->ofdm_symbol_size))) {
      ...
      LOG_I(NR_PHY,
            "TDD Normal prefix: CellId %d metric %d, phase %d, psbch %d\n",
            fp->Nid_cell,
            metric_tdd_ncp,
            phase_tdd_ncp,
            ret);
  ...
  if (ret == 0) {
    ...
    LOG_I(NR_PHY,
          ...
          ue->Mod_id,
          sync_pos,
          GET_NID1_SL(fp->Nid_SL),
          GET_NID2_SL(fp->Nid_SL),
          fp->frame_type);
