...
static void nr_csi_rs_channel_estimation(
        ...
        for (int s = 0; s < CDM_group_size; s++)  {
          ...
          for (int lp = 0; lp <= l_prime; lp++) {
            ...
            for (int kp = 0; kp <= k_prime; kp++) {
              ...
              c16_t *ls_res = csi_rs_ls_estimated_channel[ant_rx][port_tx] + kinit;
              *ls_res = c16add(*ls_res,
                               c16MulConjShift(tx_csi_rs_signal[k], rx_csi_rs_signal[k], nr_csi_info->csi_rs_generated_signal_bits));

              res->rsrpSum1 += squaredMod(rx_csi_rs_signal[k]);
...
int nr_csi_rs_channel_interpolation(
    ...
    const c16_t csi_rs_ls_estimated_channel[][N_ports][ue->frame_parms.ofdm_symbol_size],
...
static int nr_csi_rs_ri_estimation(
    const PHY_VARS_NR_UE *ue,
    const fapi_nr_dl_config_csirs_pdu_rel15_t *csirs_config_pdu,
    const nr_csi_info_t *nr_csi_info,
    const uint8_t N_ports,
...
static int nr_csi_rs_pmi_estimation(
    const PHY_VARS_NR_UE *ue,
    const fapi_nr_dl_config_csirs_pdu_rel15_t *csirs_config_pdu,
    const nr_csi_info_t *nr_csi_info,
    const uint8_t N_ports,
...
void nr_ue_csi_rs_procedures(
    ...
    const nr_csi_phy_parms_t *csi_phy_parms,
    nr_csi_symbol_res_t *res,
    c16_t csi_rs_ls_estimated_channel[ue->frame_parms.nb_antennas_rx][csi_phy_parms->N_ports][ue->frame_parms.ofdm_symbol_size])
{
  ...
  c16_t csi_rs_estimated_channel_freq[frame_parms->nb_antennas_rx][csi_phy_parms->N_ports]
