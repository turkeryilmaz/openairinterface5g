                                        ...
                                        uint32_t n_BWP_start)
{
  ...
  for (int aarx = 0; aarx < frame_parms->nb_antennas_rx; aarx++) {
    ...
    for (int rb = 0; rb < coreset_nbr_rb; rb++) {
      ...
      if ((coreset_freq_dom[rb_by6 / 8] & (1 << (7 - (rb_by6 & 7)))) == 0) {
        rb += 6;
        continue;
      }
      c16_t *rxF = NULL;
      if ((frame_parms->N_RB_DL & 1) == 0) {
        if ((rb + n_BWP_start) < frame_parms->N_RB_DL / 2)
          ...
          rxF = rxFbase + frame_parms->first_carrier_offset + RE_PER_RB * (rb + n_BWP_start);
        else
          // number of RBs is even  and c_rb is higher than half system bandwidth (we don't skip DC)
          // if these conditions are true the pointer has to be situated at the 1st part of the rxdataF
          // we point at the 1st part of the rxdataF in symbol
          rxF = rxFbase + RE_PER_RB * (rb + n_BWP_start - frame_parms->N_RB_DL / 2);
      } else {
        if ((rb + n_BWP_start) <= frame_parms->N_RB_DL / 2)
          // if RB to be treated is lower than middle system bandwidth then rxdataF pointed
