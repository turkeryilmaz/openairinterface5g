...
size_t pss_search_time_nr_candidates(const pss_search_t *p, pss_detection_result_t *candidates, size_t capacity, bool *truncated)
{
  ...
  const int first_nid2 = p->target_Nid_cell == -1 ? 0 : p->target_Nid_cell % NUMBER_PSS_SEQUENCE;
  ...
  for (int nid2 = first_nid2; nid2 < last_nid2; nid2++) {
    ...
    // Inspect one position beyond the last complete SSB to distinguish a real
    // endpoint peak from a correlation still rising toward a truncated SSB.
    // Its PSS window remains inside the capture because three symbols follow PSS.
    for (int n = 0; n <= last_start + 4; n += 4) {
