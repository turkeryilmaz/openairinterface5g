    ...
    for (int rxAnt = 0; rxAnt < frame_params->nb_antennas_rx; rxAnt++) {
      ...
      if (prs_cfg->CombSize == 2) {
        ...
        switch (k_prime) {
          ...
          default:
            LOG_I(PHY, "ERROR!! Invalid k_prime=%d for PRS comb_size %d, symbol %d\n", k_prime, prs_cfg->CombSize, l);
            return (-1);
            break;
        }
                                ...
                                uint32_t *nvar)
{
  ...
  nr_pdsch_dmrs_rx(ue, slot, gold, 1000 + p, 0, nb_rb_pdsch + rb_offset, config_type, pilot);
                                   ...
                                   int32_t *ptrs_re_symbol)
{
  ...
  *ptrs_re_symbol = 0;
  ...
  if (is_ptrs_symbol(symbol, ptrsSymbPos)) {
    ...
    const uint32_t *gold =
        nr_gold_pdsch(fp->N_RB_DL, fp->symbols_per_slot, fp->Nid_cell, nscid, nr_slot_rx, symbol);
