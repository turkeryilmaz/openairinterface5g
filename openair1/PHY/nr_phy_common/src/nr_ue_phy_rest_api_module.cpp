...
crow::json::wvalue generate_meas_json(time_stats_t *stat)
{
  return crow::json::wvalue({{"name", std::string(stat->meas_name)},
                             {"meas_enabled", stat->meas_enabled},
...
void register_routes(crow::Blueprint *bp)
{
  ...
  bp->new_rule_dynamic("/<int>/cpumeas/")
  ([](int ue_index){
    if (ue_index != 0) {
  ...
  bp->new_rule_dynamic("/<int>/cpumeas/<int>")
  ...
  bp->new_rule_dynamic("/<int>/cpumeas/<int>/enable")
  ([](int ue_index, int meas_index) {
    if (ue_index != 0) {
      return crow::response(crow::status::BAD_REQUEST);
    }
    ...
    PHY_VARS_NR_UE *phy_vars_nr_ue = PHY_vars_UE_g[ue_index][0];
    nr_ue_phy_cpu_stat_t* phy_cpu_stats = &phy_vars_nr_ue->phy_cpu_stats;
    phy_cpu_stats->cpu_time_stats[meas_index].meas_enabled = true;
    return crow::response(crow::status::OK, std::string("OK"));
  });


  bp->new_rule_dynamic("/<int>/cpumeas/<int>/disable")
  ([](int ue_index, int meas_index) {
    if (ue_index != 0) {
      return crow::response(crow::status::BAD_REQUEST);
    }
    ...
    PHY_VARS_NR_UE *phy_vars_nr_ue = PHY_vars_UE_g[ue_index][0];
    nr_ue_phy_cpu_stat_t* phy_cpu_stats = &phy_vars_nr_ue->phy_cpu_stats;
    phy_cpu_stats->cpu_time_stats[meas_index].meas_enabled = false;
    return crow::response(crow::status::OK, std::string("OK"));
  });
}
