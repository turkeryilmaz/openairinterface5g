...
// --- Main Noise Kernel (Original Version: 1 Thread per Sample) ---
...
)
{
    ...
    // Conditionally apply phase noise
    ...
    // Write back the updated state and the final output
...
// ====================================================================================
// Host-Callable Functions with C Linkage
// ====================================================================================
extern "C" {

// --- Main Noise Function (Using Pre-allocated Pinned Memory) ---
...
)
{
    ...
    // Use the pre-allocated pinned buffers passed as arguments
    float2* h_r_sig = (float2*)h_r_sig_void;
    short2* h_output_temp = (short2*)h_output_temp_void;

    // Interleave the separate real and imaginary host inputs into the pinned staging buffer
    ...
    // Copy from pinned host memory to device memory
    ...
    // Distribute the data from the pinned buffer into the final OAI buffers
    for (int ii = 0; ii < nb_rx; ii++) {
        memcpy(
