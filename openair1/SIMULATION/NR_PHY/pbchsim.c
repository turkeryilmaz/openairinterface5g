...
static bool pbchsim_sync_state_equal(const pbchsim_sync_state_t *a, const pbchsim_sync_state_t *b)
{
  return a->nid_cell == b->nid_cell && a->ssb_start_subcarrier == b->ssb_start_subcarrier && a->half_frame_bit == b->half_frame_bit
         ...
         && a->adjust_rxgain == b->adjust_rxgain;
}
                                ...
                                bool valid_pbch)
{
  ...
  // Apply phase compensation after optionally replacing the PBCH below.
  const bool phase_comp = gNB->phase_comp;
  gNB->phase_comp = false;
  nr_common_signal_procedures(gNB, 0, slot, &ssb_pdu, (fp->N_RB_DL + 63) / 64);
  gNB->phase_comp = phase_comp;
  ...
  apply_nr_rotation_TX(fp, gNB->common_vars.txdataF[0], fp->symbol_rotation[0], slot, fp->N_RB_DL, 0, fp->symbols_per_slot);
  for (int symbol = 0; symbol < fp->symbols_per_slot; symbol++)
    fftshift_inverse(gNB->common_vars.txdataF[0] + symbol * fp->ofdm_symbol_size,
                     fft_in + symbol * fp->ofdm_symbol_size,
                                    ...
                                    int ssb_subcarrier_offset)
{
  ...
  c16_t *snapshot = malloc16(capture_samples * sizeof(*snapshot));
  c16_t *capture = malloc16(capture_samples * sizeof(*capture));
  AssertFatal(capture != NULL && capture_re != NULL && capture_im != NULL && ssb_time != NULL && snapshot != NULL,
  ...
  pbchsim_quantize_capture(capture, capture_re, capture_im, capture_samples);
  memcpy(snapshot, capture, capture_samples * sizeof(*snapshot));
  ...
  c16_t *input[] = {capture};
  const nr_initial_sync_t result = nr_initial_sync(&proc, ue, capture_samples, input, gscn_info, 1);
  const pbchsim_sync_state_t state_after = pbchsim_get_sync_state(ue);
  const bool input_unchanged = memcmp(capture, snapshot, capture_samples * sizeof(*snapshot)) == 0;

  bool passed = result.cell_detected == test_case.expect_detected && input_unchanged;
  if (test_case.expect_detected) {
    const int expected_rx_offset = test_case.expected_insert_sample - native_ssb_start;
    const double cfo_error_hz = fabs(state_after.freq_offset - test_case.expected_cfo_hz);
    passed = passed && result.rx_offset == expected_rx_offset && state_after.nid_cell == test_case.expected_nid_cell
             && state_after.ssb_start_subcarrier == gscn_ssb_start_subcarrier && state_after.half_frame_bit == 0
             && state_after.ssb_index == 0 && state_after.symbol_offset == start_symbol
             && state_after.adjust_rxgain != state_before.adjust_rxgain && cfo_error_hz <= 200.0;
    printf("SSB capture regression %s: detected=%d pci=%d rx_offset=%d cfo=%.0f error=%.0f immutable=%d: %s\n",
           name,
    ...
    passed = passed && result.rx_offset == 0 && pbchsim_sync_state_equal(&state_before, &state_after);
