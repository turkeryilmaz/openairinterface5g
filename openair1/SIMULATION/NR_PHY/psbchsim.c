...
static void get_sim_cl_opts(int argc, char **argv)
{
  ...
  while ((c = getopt(argc, argv, "F:g:hIL:m:M:n:N:o:O:p:P:r:R:s:S:x:y:z:")) != -1) {
    switch (c) {
      ...
      case 'h':
        printf(
            "%s -F input_filename -g channel_mod -h(elp) -I(nitial sync) -L log_lvl -n n_frames -M SSBs -n frames -N cell_id -o FO "
            "-P phase -r seed -R RBs -s snr0 -S snr1 -y TXant -z RXant\n",
            argv[0]);
...
int main(int argc, char **argv)
{
  ...
  for (int i = 0; i < UE->frame_parms.Lmax; i++) {
    if ((SSB_positions >> i) & 0x01) {
      ...
      int frame = 0;
      int ssb_start_symbol_abs =
          (UE->slss->sl_timeoffsetssb_r16 + UE->slss->sl_timeinterval_r16 * i) * UE->frame_parms.symbols_per_slot;
      ...
      const int prb_offset = UE->frame_parms.freq_range == nr_FR1 ? UE->nrUE_config.ssb_table.ssb_offset_point_a << mu
                                                                  : UE->nrUE_config.ssb_table.ssb_offset_point_a << (mu - 2);
      ...
      UE->frame_parms.nb_prefix_samples0 =
          UE->is_synchronized_sl ? UE->frame_parms.nb_prefix_samples0 : UE->frame_parms.nb_prefix_samples;
      ...
      for (int aa = 0; aa < UE->frame_parms.nb_antennas_tx; aa++) {
        apply_nr_rotation(&UE->frame_parms,
                          ...
                          0,
                          1,
                          link_type_sl); // Conducts rotation on 0th symbol
        ...
        apply_nr_rotation(&UE->frame_parms,
                          ...
                          1,
                          13,
                          link_type_sl); // Conducts rotation on symbols located 1 (PSS) to 13 (guard)
        PHY_ofdm_mod(&UE->common_vars.txdataF[aa][UE->frame_parms.ofdm_symbol_size], // Starting at PSS (in freq)
                     (int *)&txdata[aa][UE->frame_parms.ofdm_symbol_size + UE->frame_parms.nb_prefix_samples0
  ...
  AssertFatal((((frame_length_complex_samples - 1) << 1) + 1) < 2 * frame_length_complex_samples,
              "Invalid index %d >= %d\n",
              (((frame_length_complex_samples - 1) << 1) + 1),
  ...
  for (double SNR = snr0; SNR < snr1; SNR += 0.2) {
    ...
    for (int trial = 0; trial < n_trials; trial++) {
      ...
      if (UE->is_synchronized == 0) {
        ...
        UE->symbol_offset =
            (UE->slss->sl_timeoffsetssb_r16 + UE->slss->sl_timeinterval_r16 * ssb_index) * UE->frame_parms.symbols_per_slot;
        uint8_t n_hf = 0;
        ...
        for (int i = UE->symbol_offset; i < UE->symbol_offset + 5; i++) {
          ...
          nr_psbch_channel_estimation(UE,
                                      estimateSz,
                                      dl_ch_estimates,
                                      ...
                                      0,
                                      ssb_slot,
                                      i % UE->frame_parms.symbols_per_slot,
                                      i - (UE->symbol_offset),
                                      ssb_index % 8,
                                      n_hf);
        }
        ...
        if (ret == 0) {
          ...
          for (int i = 0; i<NR_POLAR_PSBCH_PAYLOAD_BITS> > 3; i++) {
            printf("result.decoded_output[i] %d, msgDataTx.ssb[ssb_index].ssb_pdu.ssb_pdu_rel15.bchPayload %d,\n",
                   result.decoded_output[i],
                   ((msgDataTx.ssb[ssb_index].ssb_pdu.ssb_pdu_rel15.bchPayload >> (8 * (3 - i))) & 0xff));
            payload_ret +=
